...
bool ColorHarmonizationEngineGlobal::Process()
{
  ...
  for (size_t i = 0; i < _pairwiseMatches.size(); ++i)
  {
    ...
    if (bExportMask)
    {
      ...
      writeImage(out_filename_I, maskI, image::EImageColorSpace::NO_CONVERSION);
      writeImage(out_filename_J, maskJ, image::EImageColorSpace::NO_CONVERSION);
    }

    //-- Compute the histograms
    Image< RGBColor > imageI, imageJ;
    readImage(p_imaNames.first, imageI, image::EImageColorSpace::NO_CONVERSION);
    readImage(p_imaNames.second, imageJ, image::EImageColorSpace::NO_CONVERSION);
    ...
    iterSet != set_indeximage.end(); ++iterSet, ++my_progress_bar)
  {
    ...
    readImage( _fileNames[ imaNum ], image_c , image::EImageColorSpace::NO_CONVERSION);
    ...
    writeImage( out_filename, image_c , image::EImageColorSpace::NO_CONVERSION);
  }
  return true;
}
