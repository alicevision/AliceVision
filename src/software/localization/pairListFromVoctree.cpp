...
int main(int argc, char** argv)
{
  ...
  std::size_t nbMaxDescriptors = 0; ///< the file containing the list of features
  string outfile = "pairList.txt"; ///< the file in which to save the results
  bool withOutput = false; ///< flag for the optional output file
  std::size_t numImageQuery = 10; ///< the number of matches to retrieve for each image

  po::options_description desc(programDescription);
  desc.add_options()
          ("help,h", "Print this message.")
          ("verbose,v", po::value<int>(&verbosity)->default_value(1), 
            "Verbosity level, 0 to mute.")
          ("weights,w", po::value<string>(&weightsName), 
            "Input name for the weight file, if not provided the weights will be "
            "computed on the database built with the provided set.")
          ("tree,t", po::value<string>(&treeName)->required(), 
            "Input name for the tree file")
          ("keylist,l", po::value<string>(&keylist)->required(), 
            "Path to the list file (list.txt or sfm_data) generated by OpenMVG or "
            "the path to a directory containing the descriptors.")
          ("nbMaxDescriptors,m", po::value<std::size_t>(&nbMaxDescriptors)->default_value(nbMaxDescriptors), 
            "Limit the number of descriptors you load per image. Zero means no limit.")
          (",r", po::value<std::size_t>(&numImageQuery)->default_value(numImageQuery), 
            "The number of matches to retrieve for each image (If 0 it will "
            "retrieve all the matches).")
          ("outfile,o", po::value<string>(&outfile)->default_value(outfile), 
