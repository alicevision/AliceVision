...
XMPData read_xmp(const std::string& xmpFilepath, const std::string& knownPosesFilePath, const std::string& stem, fs::directory_entry pathIt)
{
    ...
    std::string distortionStr = tree.get<std::string>("x:xmpmeta.rdf:RDF.rdf:Description.xcr:DistortionCoeficients", "");
...
int aliceVision_main(int argc, char** argv)
{
        ...
        try
        {
            for (const auto& pathIt : fs::directory_iterator(knownPosesPath))
            {
                ...
                std::shared_ptr<camera::IntrinsicScaleOffsetDisto> intrinsic =
                  std::dynamic_pointer_cast<camera::IntrinsicScaleOffsetDisto>(intrinsicBase);
                ...
                if (xmp.distortionModel == "brown3t2")
                {
                    ...
                    if (camera == nullptr)
                    {
                        camera = camera::createPinhole(camera::EDISTORTION::DISTORTION_NONE, camera::EUNDISTORTION::UNDISTORTION_NONE);
    ...
    else if (is_regular_file(knownPosesPath))
    {
        ...
        if (extension == ".json")
        {
            ...
            auto it =
              std::adjacent_find(frameIdToPoseId.begin(), frameIdToPoseId.end(), [](const auto& a, const auto& b) { return a.first == b.first; });
