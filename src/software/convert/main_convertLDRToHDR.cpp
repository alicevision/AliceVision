...
inline ECalibrationMethod ECalibrationMethod_stringToEnum(const std::string& calibrationMethodName)
{
    ...
    if(methodName == "robertson")
        return ECalibrationMethod::ROBERTSON;
...
int aliceVision_main(int argc, char* argv[])
{
    ...
    optionalParams.add_options()
        ...
        ("calibrationRefineExposures", po::value<bool>(&refineExposures)->default_value(refineExposures),
         "Refine exposures provided by metadata (shutter speed, f-number, iso). Only available for 'laguerre' "
         "calibration method. Default value is set to 0.");
    ...
    std::transform(calibrationWeightFunction.begin(), calibrationWeightFunction.end(),
                   calibrationWeightFunction.begin(), ::tolower);
    ...
    if(calibrationWeightFunction == "default")
    {
        switch(calibrationMethod)
        {
            ...
            case ECalibrationMethod::DEBEVEC:
                ...
                break;
            case ECalibrationMethod::ROBERTSON:
                calibrationWeight.setRobertsonWeight();
                calibrationWeightFunctionV = "robertsonWeight";
                break;
            case ECalibrationMethod::GROSSBERG:
                break;
            case ECalibrationMethod::LAGUERRE:
...

    {
        ...
        std::string outputHtmlPath =
            (fs::path(outputPath) / (std::string("calibration_weight_") + methodName + std::string(".html"))).string();
    ...
    // calculate the response function according to the method given in argument or take the response provided by the
    // user
    {
        switch(calibrationMethod)
        {
            case ECalibrationMethod::LINEAR:
            {
...

                {
                    ...
                    std::string outputResponsePath =
                        (fs::path(outputPath) / (std::string("response_log_") + methodName + std::string(".csv")))
                            .string();
                    std::string outputResponsePathHtml =
                        (fs::path(outputPath) / (std::string("response_log_") + methodName + std::string(".html")))
                            .string();
            ...
            case ECalibrationMethod::DEBEVEC:
            {
...

                {
                    ...
                    std::string methodName = ECalibrationMethod_enumToString(calibrationMethod);
                    std::string outputResponsePath =
                        (fs::path(outputPath) / (std::string("response_invGamma_") + methodName + std::string(".csv")))
                            .string();
                    std::string outputResponsePathHtml =
                        (fs::path(outputPath) / (std::string("response_invGamma_") + methodName + std::string(".html")))
                            .string();
            ...
            case ECalibrationMethod::ROBERTSON:
            {
                ...
                if(calibrationNbPoints <= 0)
                  calibrationNbPoints = 1000000;
                calibration.process(groupedFilenames, channelQuantization, groupedExposures, calibrationNbPoints,
                fisheye, calibrationWeight, response); response.scale();
                */
            }
            break;
            case ECalibrationMethod::GROSSBERG:
...

    {
        std::string methodName = ECalibrationMethod_enumToString(calibrationMethod);
        std::string outputResponsePath =
            (fs::path(outputPath) / (std::string("response_") + methodName + std::string(".csv"))).string();
        std::string outputResponsePathHtml =
            (fs::path(outputPath) / (std::string("response_") + methodName + std::string(".html"))).string();
...

    {
        ...
        std::string outputHtmlPath =
            (fs::path(outputPath) / (std::string("fusion_weight_") + methodName + std::string(".html"))).string();
    ...
    for(int g = 0; g < groupedFilenames.size(); ++g)
    {
        ...
        {
            merge.postProcessHighlight(images, groupedExposures[g], fusionWeight, response, HDRimage,
                                       targetCameraExposure, highlightCorrectionFactor, highlightTargetLux);
        }
