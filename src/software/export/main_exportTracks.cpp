...
int aliceVision_main(int argc, char** argv)
{
    ...
    auto myProgressBar = system::createConsoleProgressDisplay((viewCount * (viewCount - 1)) / 2.0, std::cout, "Export pairwise tracks\n");
    ...
    for (std::size_t I = 0; I < viewCount; ++I)
    {
        ...
        for (std::size_t J = I + 1; J < viewCount; ++J, ++myProgressBar)
        {
            ...
            else
            {
                ...
                for (track::TracksMap::const_iterator tracksIt = mapTracksCommon.begin(); tracksIt != mapTracksCommon.end(); ++tracksIt)
                {
                ...
                fs::path outputFilename =
                  fs::path(outputFolder) / std::string(std::to_string(viewI->getViewId()) + "_" + std::to_string(viewJ->getViewId()) + "_" +
                                                       std::to_string(mapTracksCommon.size()) + ".svg");
