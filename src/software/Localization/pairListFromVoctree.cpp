...
int main(int argc, char** argv)
{
  ...
  string previousKeylist; //the file containing the list of features
  bool withPreviousKeylist = false; ///< flag for the optional previous keylist file
  ...
  desc.add_options()
          ...
          ("previousKeylist,p", po::value<string>(&previousKeylist), "Path to the previous keylist file (list.txt or sfm_data generated by openMVG")
  ...
  if(vm.count("previousKeylist"))
  {
    withPreviousKeylist = true;
  }
  //**********************************************************
  // Load the voctree
  //**********************************************************
  ...
  if (withPreviousKeylist)
  {
    POPART_COUT("Reading descriptors from " << previousKeylist);
    
    DocumentMap previousDocuments;
    auto detect_start = std::chrono::steady_clock::now();
    size_t numTotFeatures = openMVG::voctree::populateDatabase<DescriptorUChar>(previousKeylist, tree, db, previousDocuments);
