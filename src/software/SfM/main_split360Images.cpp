...
bool splitDualFisheye(const std::string& imagePath, const std::string& outputDirectory, const std::string& splitPreset)
{
  ...
  OPENMVG_LOG_INFO(imagePath + " successfully split");
  return true;
}
...
bool splitEquirectangular(const std::string& imagePath, const std::string& outputDirectory, std::size_t nbSplits, std::size_t splitResolution)
{
  ...
  {
    OPENMVG_LOG_WARNING("Warning: Cannot read the image '" << imagePath << "'" << std::endl);
    return false;
  }
...
int main(int argc, char** argv)
{
  ...
  //check split mode
  {
    ...
    {
      OPENMVG_CERR("ERROR: invalid split mode : " << splitMode);
      return EXIT_FAILURE;
    }
  }
  ...
  //check dual-fisheye split preset
  {
    ...
    {
      OPENMVG_CERR("ERROR: invalid dual-fisheye split preset : " << dualFisheyeSplitPreset);
      return EXIT_FAILURE;
    }
  }
  ...
  // debugging prints, print out all the parameters
  {
    OPENMVG_COUT("Program called with the following parameters:"                            << std::endl
                 << "\timagePath : "                      << inputPath                      << std::endl
                 << "\toutputDirectory : "                << outputDirectory                << std::endl
                 << "\tsplitMode : "                      << splitMode                      << std::endl
                 << "\tdualFisheyeSplitPreset : "         << dualFisheyeSplitPreset         << std::endl
                 << "\tequirectangularNbSplits : "        << equirectangularNbSplits        << std::endl
                 << "\tequirectangularSplitResolution : " << equirectangularSplitResolution << std::endl
                 << "\tequirectangularDemoMode : "        << equirectangularDemoMode        << std::endl);
  }
...

  {
    ...
    if(bfs::exists(path) && bfs::is_directory(path))
    {
      ...
      OPENMVG_LOG_INFO("Find " << imagePaths.size() << " file paths.");
    }
    ...
    {
      OPENMVG_CERR("ERROR: can't find file or directory " << inputPath);
      return EXIT_FAILURE;
    }
  }
  ...
  for(const std::string& imagePath : imagePaths)
  {
    ...
    {
      OPENMVG_LOG_ERROR("Exif mode not implemented yet !");
    }
  ...
  if(!badPaths.empty())
  {
    OPENMVG_LOG_ERROR("Error: Can't open image file(s) below");
    for(const std::string& imagePath : imagePaths)
       OPENMVG_LOG_ERROR("\t - " << imagePath);
  }
