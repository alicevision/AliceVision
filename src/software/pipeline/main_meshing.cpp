        ...
        if(!ptsCams[i].empty())
        {
            for(int cam : ptsCams[i])
            {
                ...
                const sfmData::Observation observation(
                    intrinsicPtr->project(sfmData.getPose(view).getTransform(), pt3D.homogeneous(), true),
                    UndefinedIndexT, unknownScale); // apply distortion
...
int aliceVision_main(int argc, char* argv[])
{
    ...
    requiredParams.add_options()("input,i", po::value<std::string>(&sfmDataFilename)->required(), "SfMData file.")(
        ...
        "Output Dense SfMData file.")("outputMesh,o", po::value<std::string>(&outputMesh)->required(), "Output mesh");

    po::options_description optionalParams("Optional parameters");
    optionalParams.add_options()("depthMapsFolder", po::value<std::string>(&depthMapsFolder),
        ...
        "boundingBoxRescale", po::value<float>(&boundingBoxRescale),
        "Scale factor used to enlarge or reduce the automatically computed bounding box.")(
        ...
        "maxPointsPerVoxel", po::value<int>(&maxPtsPerVoxel)->default_value(maxPtsPerVoxel), "Max points per voxel.")(
        "minStep", po::value<int>(&fuseParams.minStep)->default_value(fuseParams.minStep),
        "The step used to load depth values from depth maps is computed from maxInputPts. Here we define the minimal "
        "value for this step, "
        ...
        "simFactor")("angleFactor", po::value<float>(&fuseParams.angleFactor)->default_value(fuseParams.angleFactor),
                     "angleFactor")("minVis", po::value<int>(&fuseParams.minVis)->default_value(fuseParams.minVis),
        ...
        "Add SfM Landmarks into the dense point cloud (created from depth maps). If only the SfM is provided in input, "
        "SfM landmarks will be used regardless of this option.")(
    ...
    advancedParams.add_options()("universePercentile",
                                 po::value<double>(&universePercentile)->default_value(universePercentile),
                                 "universe percentile")(
        ...
        "simGaussianSizeInit")("simGaussianSize",
        ...
        "minAngleThreshold")(
        "refineFuse", po::value<bool>(&fuseParams.refineFuse)->default_value(fuseParams.refineFuse), "refineFuse")(
        "helperPointsGridSize", po::value<int>(&helperPointsGridSize)->default_value(helperPointsGridSize),
        "Helper points grid size.")("densifyNbFront", po::value<int>(&densifyNbFront)->default_value(densifyNbFront),
        ...
        po::value<float>(&fuseParams.maskHelperPointsWeight)->default_value(fuseParams.maskHelperPointsWeight),
        ...
        "fullWeight", po::value<double>(&fullWeight)->default_value(fullWeight), "Weighting of the FULL cells.")(
        ...
        po::value<bool>(&voteFilteringForWeaklySupportedSurfaces)
            ->default_value(voteFilteringForWeaklySupportedSurfaces),
        ...
        po::value<int>(&invertTetrahedronBasedOnNeighborsNbIterations)
            ->default_value(invertTetrahedronBasedOnNeighborsNbIterations),
        ...
        "Filter cells status on surface around vertices to improve smoothness using solid angle ratio between "
        "full/empty parts.")(
        "nbSolidAngleFilteringIterations",
        po::value<int>(&nbSolidAngleFilteringIterations)->default_value(nbSolidAngleFilteringIterations),
        "Number of iterations to filter the status cells based on solid angle ratio.")(
        "maxNbConnectedHelperPoints",
        po::value<int>(&maxNbConnectedHelperPoints)->default_value(maxNbConnectedHelperPoints),
        "Maximum number of connected helper points before we remove them.")(
        "exportDebugTetrahedralization",
        po::value<bool>(&exportDebugTetrahedralization)->default_value(exportDebugTetrahedralization),
        "Export debug cells score as tetrahedral mesh. WARNING: could create huge meshes, only use on very small "
        "datasets.")("seed", po::value<unsigned int>(&seed)->default_value(seed),
    ...
    {
        if(depthMapsFolder.empty() && repartitionMode == eRepartitionMultiResolution &&
        ...
        else
        {
            ALICEVISION_LOG_ERROR("Invalid input options:\n"
                                  ...
                                  "- Meshing from SfM require option --partitioning set to 'singleBlock' and option "
                                  "--repartition set to 'multiResolution'.");
            return EXIT_FAILURE;
        }
    }
    ...
    mp.userParams.put("hallucinationsFiltering.invertTetrahedronBasedOnNeighborsNbIterations",
                      invertTetrahedronBasedOnNeighborsNbIterations);
        ...
        case eRepartitionMultiResolution:
        {
            switch(partitioningMode)
            {
                ...
                case ePartitioningSingleBlock:
                {
                    ...
                    // option to enlarge or reduce the bounding box
                    if(boundingBoxRescale != 1)
                    {
                        aliceVision::Point3d center;
                        //compute bb center
                        for(int i = 0; i < 6; ++i)
                            center += hexah[i];
                        center /= 6.0;
                        //center, scale and decenter
                        for(int i = 0; i < 6; ++i)
                        {
                            hexah[i] -= center;
                            hexah[i] *= boundingBoxRescale;
                            hexah[i] += center;
                        }
                        const double length = hexah[0].x - hexah[1].x;
                        const double width = hexah[0].y - hexah[3].y;
