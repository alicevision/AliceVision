  ...
  for(std::size_t i = 0; i < vertices.size(); ++i)
  {
    ...
    if(!ptsCams[i].empty())
    {
      for(int cam : ptsCams[i])
      {
        ...
        const sfmData::Observation observation(intrinsicPtr->project(sfmData.getComputedPose(view).getTransform(), pt3D.homogeneous(), true), UndefinedIndexT, unknownScale); // apply distortion
