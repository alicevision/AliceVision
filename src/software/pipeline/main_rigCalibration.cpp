...
int aliceVision_main(int argc, char** argv)
{
    ...
    commonParams.add_options()
        ...
        ("reprojectionError", po::value<double>(&resectionErrorMax)->default_value(resectionErrorMax),
         "Maximum reprojection error (in pixels) allowed for resectioning. If set to 0, "
         "it lets the ACRansac select an optimal value.")
    ...
    for (std::size_t idCamera = 0; idCamera < numCameras; ++idCamera)
    {
        ...
        {
            ALICEVISION_COUT("******************************");
            ALICEVISION_COUT("Stream " << idCamera << " Frame " << utils::toStringZeroPadded(currentFrame, 4) << "/" << nbFrames << " : ("
                                       << iInputFrame << "/" << nbFramesToProcess << ")");
            ALICEVISION_COUT("******************************");
            auto detect_start = std::chrono::steady_clock::now();
            localization::LocalizationResult localizationResult;
            localizer->setCudaPipe(idCamera);
            const bool ok = localizer->localize(
              imageGrey, param.get(), randomNumberGenerator, hasIntrinsics /*useInputIntrinsics*/, *queryIntrinsics, localizationResult);
...

    {
        ...
        for (std::size_t idCam = 0; idCam < numRigCam; ++idCam)
        {
            ...
            ALICEVISION_COUT("Camera " << idCam << " localized " << numLocalized << "/" << currResult.size());
        }
    }
