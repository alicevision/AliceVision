 ...
 */
bool listFiles(const fs::path& folderOrFile, const std::vector<std::string>& extensions, std::vector<fs::path>& resources)
{
...
inline std::ostream& operator<<(std::ostream& os, EGroupCameraFallback s) { return os << EGroupCameraFallback_enumToString(s); }
...
int aliceVision_main(int argc, char** argv)
{
    ...
    std::map<std::pair<std::string, std::string>, std::pair<std::string, aliceVision::sensorDB::Datasheet>>
      unsureSensors;                                            // key (make,model), value (first imagePath,datasheet)
...
#pragma omp critical(rig)
            ...
            {
                ALICEVISION_LOG_WARNING("Invalid rig structure for view: " << view.getImage().getImagePath() << std::endl
                                                                           << e.what() << std::endl
                                                                           << "Used as single image.");
            }
        }
...
#pragma omp critical(dcp)
        ...
        if (isRaw)
        {
            ...
            if (!dcpError)
            {
                view.getImage().addMetadata("AliceVision:rawColorInterpretation",
                                            image::ERawColorInterpretation_enumToString(rawColorInterpretation));
            }
            else
            {
                view.getImage().addMetadata("AliceVision:rawColorInterpretation",
                                            image::ERawColorInterpretation_enumToString(image::ERawColorInterpretation::LibRawWhiteBalancing));
                if (!dcpDatabase.empty())
                    ALICEVISION_LOG_WARNING("DCP Profile not found for image: " << view.getImage().getImagePath()
                                                                                << ". Use LibRawWhiteBalancing option for raw color processing.");
            }
        }
...
#pragma omp critical(missingDeviceUID)
                ...
                if (view.isPartOfRig())
                {
                    // when we have no unique camera identifier, so for rig images, we ensure that each camera of the rig have different serial
                    // numbers.
                    intrinsic->setSerialNumber(intrinsic->serialNumber() + "_rig_" + std::to_string(view.getRigId()) + "_" +
                                               std::to_string(view.getSubPoseId()));
                }
            }
...
#pragma omp critical(intrinsics)
    ...
    if (!detectedRigs.empty())
    {
        for (const auto& subPosesPerRigId : detectedRigs)
        {
            ...
            for (const auto& nbPosePerSubPoseId : subPosesPerRigId.second)
            {
                ...
                {
                    ALICEVISION_LOG_ERROR("Wrong subPoseId in rig structure: it should be an index not a random number (subPoseId: "
                                          << nbPosePerSubPoseId.first << ", number of subposes: " << nbSubPose << ").");
                    return EXIT_FAILURE;
                }
                ...
                {
                    ALICEVISION_LOG_WARNING("Wrong number of poses per subPose in detected rig structure (" << nbPosePerSubPoseId.second
                                                                                                            << " != " << nbPoses << ").");
                }
            }
    ...
    if (completeViewCount < 1 || (completeViewCount < 2 && !allowSingleView))
    {
        ALICEVISION_LOG_ERROR(
          "At least " << std::string(allowSingleView ? "one image" : "two images") << " should have an initialized intrinsic." << std::endl
    ...
    ALICEVISION_LOG_INFO("CameraInit report:"
                         << "\n\t- # Views: " << sfmData.getViews().size() << "\n\t   - # with focal length initialization (from metadata): "
