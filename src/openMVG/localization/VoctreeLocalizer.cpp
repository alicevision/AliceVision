                                                ...
                                                LocalizationResult &localizationResult, const std::string& imagePath /*= std::string()*/)
{
  ...
  OPENMVG_COUT("[features]\tExtract SIFT done: found " 
...
//  typedef matching::ArrayMatcher_Kdtree_Flann<unsigned char, MetricT> MatcherT;
  ...
  for(const voctree::DocMatch& matchedImage : matchedImages)
  {
    ...
    if(!refineStatus)
      OPENMVG_COUT("[poseEstimation]\tRefine pose could not improve the estimation of the camera pose.");
...
//      OPENMVG_COUT("[matching]\tMatching with " << matchedView->s_Img_path << " failed! Skipping image");
  ...
  bool refineStatus = sfm::SfM_Localizer::RefinePose(&queryIntrinsics, 
                                                     ...
                                                     param._refineIntrinsics /*b_refine_intrinsic*/);
  if(!refineStatus)
    OPENMVG_COUT("Refine pose could not improve the estimation of the camera pose.");
                    ...
                    std::vector<matching::IndMatch> & vec_featureMatches) const
{
  ...
  if (!matchWorked)
  {
    OPENMVG_COUT("\tRobust matching failed!");
    return false;
  }
  assert(vec_featureMatches.size()>0);
  ...
  if(!valid)
  {
    OPENMVG_COUT("[matching]\tUnable to robustly matching the query image with the database image.");
    return false;
  }
