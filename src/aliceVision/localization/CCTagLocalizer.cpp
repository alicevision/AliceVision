...
CCTagLocalizer::CCTagLocalizer(const sfmData::SfMData& sfmData, const std::string& descriptorsFolder)
  : _cudaPipe(0)
{
    _sfm_data = sfmData;
...
bool CCTagLocalizer::loadReconstructionDescriptors(const sfmData::SfMData& sfm_data, const std::string& feat_directory)
{
    ...
    for (const auto& iter : _sfm_data.getViews())
    {
        ...
        const auto& observations = observationsPerView.at(id_view);
        {
            ...
            _regionsPerView.getData()[id_view][descType] =
              createFilteredRegions(*currRegions, observations.at(descType), _reconstructedRegionsMappingPerView[id_view][descType]);
        }
    }

    {
        ...
        ALICEVISION_LOG_DEBUG("Found " << presentCCtagIds.size() << " different CCTag ids in the database with " << _sfm_data.getLandmarks().size()
...
void CCTagLocalizer::setCudaPipe(int i) { _cudaPipe = i; }
                              ...
                              const std::string& imagePath)
{
    ...
    system::Timer timer;
    getAllAssociations(
      queryRegions, imageSize, *param, randomNumberGenerator, occurences, resectionData.pt2D, resectionData.pt3D, matchedImages, imagePath);
                                        ...
                                        std::vector<LocalizationResult>& vec_locResults)
{
    ...
    const EstimationStatus status =
      rigResection(vec_pts2D, vec_pts3D, vec_queryIntrinsics, vec_subPoses, nullptr, rigPose, vec_inliers, param->_angularThreshold);
                                        ...
                                        const std::string& imagePath) const
{
    ...
    kNearestKeyFrames(queryRegions, _cctagDescType, _regionsPerView, param._nNearestKeyFrames, nearestKeyFrames);
...

    {
        ...
        for (const auto& idx : out_occurences)
        {
            ...
            ALICEVISION_LOG_DEBUG("[matching]\tAssociations " << feature::EImageDescriberType_enumToString(key.descType) << " " << key.landmarkId
                                                              << "," << key.featId << " found " << value << " times.");
            if (value > maxOcc)
                maxOcc = value;
        }
...
void viewMatching(const feature::CCTAG_Regions& regionsA, const feature::CCTAG_Regions& regionsB, std::vector<matching::IndMatch>& out_featureMatches)
{
...
float viewSimilarity(const feature::CCTAG_Regions& regionsA, const feature::CCTAG_Regions& regionsB)
{
