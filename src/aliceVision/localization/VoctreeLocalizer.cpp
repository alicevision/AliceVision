...
std::ostream& operator<<(std::ostream& os, VoctreeLocalizer::Algorithm a)
{
    switch (a)
    {
        case VoctreeLocalizer::Algorithm::FirstBest:
            os << "FirstBest";
            break;
        case VoctreeLocalizer::Algorithm::BestResult:
            os << "BestResult";
            break;
        case VoctreeLocalizer::Algorithm::AllResults:
            os << "AllResults";
            break;
        case VoctreeLocalizer::Algorithm::Cluster:
            os << "Cluster";
            break;
        default:
                                ...
                                const std::string& imagePath)
{
    ...
    switch (voctreeParam->_algorithm)
    {
        case Algorithm::FirstBest:
            return localizeFirstBestResult(
              queryRegions, imageSize, *voctreeParam, randomNumberGenerator, useInputIntrinsics, queryIntrinsics, localizationResult, imagePath);
        case Algorithm::BestResult:
            throw std::invalid_argument("BestResult not yet implemented");
        case Algorithm::AllResults:
            return localizeAllResults(
              queryRegions, imageSize, *voctreeParam, randomNumberGenerator, useInputIntrinsics, queryIntrinsics, localizationResult, imagePath);
                                ...
                                const std::string& imagePath /* = std::string() */)
{
    ...
    for (const auto& imageDescriber : _imageDescribers)
    {
        ...
        ALICEVISION_LOG_DEBUG("[features]\tExtract " << feature::EImageDescriberType_enumToString(descType) << " done: found "
    ...
    return localize(
      queryRegionsPerDesc, queryImageSize, param, randomNumberGenerator, useInputIntrinsics, queryIntrinsics, localizationResult, imagePath);
}

bool VoctreeLocalizer::loadReconstructionDescriptors(const sfmData::SfMData& sfm_data, const std::string& feat_directory)
{
 ...
 */
bool VoctreeLocalizer::initDatabase(const std::string& vocTreeFilepath, const std::string& weightsFilepath, const std::string& featFolder)
{
    ...
    ALICEVISION_LOG_DEBUG("tree loaded with " << _voctree->levels() << " levels and " << _voctree->splits() << " branching factors");
                                               ...
                                               const std::string& imagePath)
{
    ...
    for (const voctree::DocMatch& matchedImage : matchedImages)
    {
        ...
        {
            ALICEVISION_LOG_DEBUG("[matching]\tSkipping matching with " << matchedView->getImage().getImagePath()
                                                                        << " as it has too few visible 3D points ("
                                                                        << _regionsPerView.getRegionsPerDesc(matchedViewId).getNbAllRegions() << ")");
            continue;
        }
        ...
        if (!isPinhole(matchedIntrinsicsBase->getType()))
            throw std::logic_error("Unsupported intrinsic: " + EINTRINSIC_enumToString(matchedIntrinsicsBase->getType()) +
                                   " (only Pinhole cameras are supported for localization).");
...

        {
            ...
            const geometry::Pose3 referencePose = _sfm_data.getComputedPose(*_sfm_data.getViews().at(matchedViewId)).getTransform();
            ...
            ALICEVISION_LOG_DEBUG("angular difference: " << radianToDegree(getRotationMagnitude(pose.rotation() * referencePose.rotation().inverse()))
                                                         << "deg");
            ALICEVISION_LOG_DEBUG("center difference: " << (pose.center() - referencePose.center()).norm());
            ALICEVISION_LOG_DEBUG("err = [err; " << radianToDegree(getRotationMagnitude(pose.rotation() * referencePose.rotation().inverse())) << ", "
                                                 << (pose.center() - referencePose.center()).norm() << "];");
        }
                                          ...
                                          const std::string& imagePath)
{
...

    {
        ...
        ALICEVISION_LOG_DEBUG("RMSE = " << std::sqrt(sqrErrors.mean()) << " min = " << std::sqrt(sqrErrors.minCoeff())
                                          ...
                                          const std::string& imagePath) const
{
    ...
    {
        ALICEVISION_LOG_WARNING("[database]\t No feature type " << feature::EImageDescriberType_enumToString(_voctreeDescType)
                                                                << " in query region.");
        return;
    }
    voctree::SparseHistogram requestImageWords = _voctree->quantizeToSparse(queryRegions.at(_voctreeDescType)->blindDescriptors());
    ...
    if (param._nbFrameBufferMatching > 0)
    {
        ALICEVISION_LOG_DEBUG("[matching]\tUsing frameBuffer matching: matching with the past " << param._nbFrameBufferMatching << " frames");
...

    {
        ...
        for (std::size_t value = 1; value < numCollectedPts; ++value)
        {
            ...
            ALICEVISION_LOG_DEBUG("[matching]\tThere are " << counter << " associations occurred " << value << " times ("
...
bool VoctreeLocalizer::robustMatching(
  matching::RegionsDatabaseMatcherPerDesc& matchers,
  ...
  robustEstimation::ERobustEstimator estimator) const
{
    ...
    EstimationStatus estimationState = geometricFilter.geometricEstimation(matchers.getDatabaseRegionsPerDesc(),
                                                                           matchedRegions,
                                                                           queryIntrinsics,
                                                                           matchedIntrinsics,
                                                                           imageSizeI,
                                   ...
                                   std::vector<LocalizationResult>& vec_locResults)
{
    ...
    for (size_t i = 0; i < numCams; ++i)
    {
        ...
        {
            ALICEVISION_LOG_DEBUG("[features]\tExtract " << feature::EImageDescriberType_enumToString(imageDescriber->getDescriberType())
                                                         << " from query image...");
    ...
    return localizeRig(
      vec_queryRegions, vec_imageSize, parameters, randomNumberGenerator, vec_queryIntrinsics, vec_subPoses, rigPose, vec_locResults);
}
                                          ...
                                          std::vector<LocalizationResult>& vec_locResults)
{
    ...
    const EstimationStatus resectionEstimation =
      rigResection(vec_pts2D, vec_pts3D, vec_queryIntrinsics, vec_subPoses, &descTypesPerCamera, rigPose, vec_inliers, param->_angularThreshold);
    ...
    {  // just debugging stuff, this block can be removed
        ...
        if (vec_inliers.size() < numCams)
        {
            ...
            ALICEVISION_CERR("WARNING: RIG Voctree Localizer: Inliers in " << vec_inliers.size() << " cameras on a RIG of " << numCams
                                                                           << " cameras.");
        }
    ...
    const auto& resInl = computeInliers(vec_pts2D, vec_pts3D, vec_queryIntrinsics, vec_subPoses, param->_errorMax, rigPose, vec_inliers);

    ALICEVISION_LOG_DEBUG("After first recomputation of inliers with a threshold of " << param->_errorMax << " the RMSE is: " << resInl.first);

    aliceVision::system::Timer timer;
    const std::size_t minNumPoints = 4;
    const bool refineOk =
      iterativeRefineRigPose(vec_pts2D, vec_pts3D, vec_queryIntrinsics, vec_subPoses, param->_errorMax, minNumPoints, vec_inliers, rigPose);
                                         ...
                                         std::vector<LocalizationResult>& vec_localizationResults)
{
    ...
    for (size_t i = 0; i < numCams; ++i)
    {
        isLocalized[i] = localize(vec_queryRegions[i],
                                  vec_imageSize[i],
                                  parameters,
                                  randomNumberGenerator,
                                  true /*useInputIntrinsics*/,
                                  vec_queryIntrinsics[i],
                                  vec_localizationResults[i]);
    ...
    else
    {
        ...
        const std::size_t idx = std::distance(isLocalized.begin(), std::find(isLocalized.begin(), isLocalized.end(), true));
