                              ...
                              bool verbosity /*= true*/)
{
    ...
    for (std::size_t cam = 0; cam < numCameras; ++cam)
    {
        ...
        for (std::size_t i = 0; i < numPts; ++i)
        {
            ...
            auto pt = currCamera.ima2cam(currCamera.getUndistortedPixel(pts2d[cam].col(i)));
    ...
    absolute_pose::NoncentralAbsoluteAdapter adapter(bearingVectors, camCorrespondences, points, camOffsets, camRotations);
    ...
    std::shared_ptr<sac_problems::absolute_pose::AbsolutePoseSacProblem> absposeproblem_ptr(
      new sac_problems::absolute_pose::AbsolutePoseSacProblem(adapter, sac_problems::absolute_pose::AbsolutePoseSacProblem::GP3P));
    ...
    if (verbosity)
    {
        ALICEVISION_LOG_DEBUG("-------------------------------"
                              ...
                              << "\n"
                                 "-- #Thresehold used: "
                              << ransac.threshold_ << " (" << radianToDegree(angularThreshold)
                              << "deg)\n"
                                 "-- Time spent in ransac [ms]: "
                              << detect_elapsed.count()
                              << "\n"
                                 "-------------------------------");
    }
