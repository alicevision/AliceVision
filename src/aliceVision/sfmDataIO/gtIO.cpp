                             ...
                             camera::Pinhole& cam, geometry::Pose3& pose)
{
  ...
  if (vfs::path(camName).extension().string() == ".bin")
  {
    auto in = fs.open_read_binary(camName);
    if (!in.is_open())
    {
  ...
  {
    auto ifs = fs.open_read_text(camName);
    if (!ifs.is_open()) {
      ALICEVISION_LOG_ERROR("Failed to open file '" << camName << "' for reading");
      return false;
    }
...
**/
bool readGt(vfs::filesystem& fs, const std::string& rootPath, sfmData::SfMData& sfmData, bool useUID)
{
  // IF GT_Folder exists, perform evaluation of the quality of rotation estimates
  const std::string sGTPath = (vfs::path(rootPath) / "gt_dense_cameras").string();
  ...
  bool (*fcnReadCamPtr)(vfs::filesystem&, const std::string &, camera::Pinhole &, geometry::Pose3&);
  std::string suffix;
  ...
  for (vfs::directory_iterator i(fs, sGTPath); i != endItr; ++i)
  {
  ...
  const std::string imgPath = (vfs::path(rootPath) / "images").string();
    ...
    iter != vec_camfilenames.end(); ++iter, ++index)
  {
    ...
    bool loaded = fcnReadCamPtr(fs, (vfs::path(sGTPath) / *iter).string(), *pinholeIntrinsic.get(), pose);
    if (!loaded)
    {
    ...
    const std::string imgName = vfs::path(*iter).stem().string();
    const std::string imgFile = (vfs::path(imgPath) / imgName).string();

    std::shared_ptr<sfmData::View> viewPtr = std::make_shared<sfmData::View>(imgFile, UndefinedIndexT, index);

    updateIncompleteView(fs, *viewPtr, EViewIdMethod::FILENAME, ".*?(\\d+)");
