...
BOOST_AUTO_TEST_CASE(kmeanInitializerVarying)
{
    ...
    for (size_t trial = 0; trial < numTrial; ++trial)
    {
        ...
        for (std::size_t i = 0; i < K; ++i)
        {
            ...
            for (std::size_t j = 0; j < FEATURENUMBER; ++j)
            {
                features.push_back((FeatureFloat::Random(DIMENSION) + FeatureFloat::Constant(DIMENSION, STEP * i) -
                                    FeatureFloat::Constant(DIMENSION, STEP * (K - 1) / 2)) /
                                   ((STEP * (K - 1) / 2) * sqrt(DIMENSION)));
...
BOOST_AUTO_TEST_CASE(kmeanSimple)
{
    ...
    for (std::size_t trial = 0; trial < 10; ++trial)
    {
        ...
        for (std::size_t i = 0; i < K; ++i)
        {
            ...
            for (std::size_t j = 0; j < FEATURENUMBER; ++j)
            {
                features.push_back((FeatureFloat::Random(1, DIMENSION) + Eigen::MatrixXf::Constant(1, DIMENSION, STEP * i) -
                                    Eigen::MatrixXf::Constant(1, DIMENSION, STEP * (K - 1) / 2)) /
                                   ((STEP * (K - 1) / 2) * sqrt(DIMENSION)));
                BOOST_CHECK(voctree::checkElements(features[j], "init"));
            }
            centersGT.push_back((Eigen::MatrixXf::Constant(1, DIMENSION, STEP * i) - Eigen::MatrixXf::Constant(1, DIMENSION, STEP * (K - 1) / 2)) /
                                ((STEP * (K - 1) / 2) * sqrt(DIMENSION)));
        }
        ...
        for (std::size_t i = 0; i < K; ++i)
        {
            voctree::SimpleKmeans<FeatureFloat>::squared_distance_type bestDist =
              std::numeric_limits<voctree::SimpleKmeans<FeatureFloat>::squared_distance_type>::max();
...
BOOST_AUTO_TEST_CASE(kmeanVarying)
{
    ...
    for (std::size_t trial = 0; trial < numTrial; ++trial)
    {
        ...
        for (std::size_t i = 0; i < K; ++i)
        {
            ...
            centersGT.push_back((FeatureFloat::Constant(DIMENSION, STEP * i) - FeatureFloat::Constant(DIMENSION, STEP * (K - 1) / 2)) /
                                ((STEP * (K - 1) / 2) * sqrt(DIMENSION)));
        }
