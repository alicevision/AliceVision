...
class VocabularyTree : public IVocabularyTree
{
  public:
    ...
    {
        return (centers_ == other.centers_) && (valid_centers_ == other.valid_centers_) && (k_ == other.k_) && (levels_ == other.levels_) &&
               (num_words_ == other.num_words_) && (word_start_ == other.word_start_);
    }
 ...
 */
float sparseDistance(const SparseHistogram& v1,
                     const SparseHistogram& v2,
                     const std::string& distanceMethod = "classic",
                     const std::vector<float>& word_weights = std::vector<float>());
...
inline std::unique_ptr<IVocabularyTree> createVoctreeForDescriberType(feature::EImageDescriberType imageDescriberType)
{
    ...
    switch (imageDescriberType)
    {
        ...
        case EImageDescriberType::SIFT_FLOAT:
            res.reset(new VocabularyTree<SIFT_Float_Regions::DescriptorT>);
            break;
        ...
        case EImageDescriberType::CCTAG4:
            res.reset(new VocabularyTree<CCTAG_Regions::DescriptorT>);
            break;
