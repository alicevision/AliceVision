...
BOOST_AUTO_TEST_CASE(EuclideanResection_Points6AllRandomInput)
{
    ...
    Mat3 KK;
    KK << 2796, 0, 800, 0, 2796, 600, 0, 0, 1;
    ...
    Mat3 R_input;
    R_input = Eigen::AngleAxisd(rand(), Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(rand(), Eigen::Vector3d::UnitY()) *
    ...
    Vec3 T_expected;
    Mat3X X_world;
    CreateCameraSystem(KK, x_image, X_distances, R_input, T_input, &x_camera, &X_world, &R_expected, &T_expected);
