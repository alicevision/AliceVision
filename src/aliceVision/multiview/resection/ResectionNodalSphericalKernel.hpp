...
class ResectionNodalSphericalKernel
{
public:
    using ModelT = robustEstimation::Mat3Model;

public:
    ResectionNodalSphericalKernel(std::shared_ptr<camera::IntrinsicBase> & camera, 
    ...
    : _camera(camera)
    {
        ...
        {
            _structure.push_back(pt.normalized());
        }
    ...
    void errors(const robustEstimation::Mat3Model & model, std::vector<double>& errors) const 
    {
        ...
        for (int idx = 0; idx < _structure.size(); idx++)
        {
            ...
            const Vec2 residual = _camera->residual(geometry::Pose3(R, Vec3::Zero()), X, x, false);
