...
void absoluteOrientation(const Mat3X& X, const Mat3X& Xp, Mat3* R, Vec3* t)
{
    ...
    Mat4 N;
    N << Sxx + Syy + Szz, Syz - Szy, Szx - Sxz, Sxy - Syx, Syz - Szy, Sxx - Syy - Szz, Sxy + Syx, Szx + Sxz, Szx - Sxz, Sxy + Syx, -Sxx + Syy - Szz,
      Syz + Szy, Sxy - Syx, Szx + Sxz, Syz + Szy, -Sxx - Syy + Szz;
    ...
    (*R) << qq(0) + qq(1) - qq(2) - qq(3), 2 * (q1q2 - q0q3), 2 * (q1q3 + q0q2), 2 * (q1q2 + q0q3), qq(0) - qq(1) + qq(2) - qq(3), 2 * (q2q3 - q0q1),
      2 * (q1q3 - q0q2), 2 * (q2q3 + q0q1), qq(0) - qq(1) - qq(2) + qq(3);
...
bool EPnPSolver::resection(const Mat2X& x2d, const Mat3X& x3d, Mat3* R, Vec3* t) const
{
    ...
    for (std::size_t c = 0; c < nbPoints; ++c)
    {
        ...
        M.block(2 * c, 0, 2, 12) << a0, 0, a0 * (-ui), a1, 0, a1 * (-ui), a2, 0, a2 * (-ui), a3, 0, a3 * (-ui), 0, a0, a0 * (-vi), 0, a1, a1 * (-vi),
          0, a2, a2 * (-vi), 0, a3, a3 * (-vi);
    }
    ...
    {
        L.row(r) << dv1.row(r).dot(dv1.row(r)), 2.0 * dv1.row(r).dot(dv2.row(r)), dv2.row(r).dot(dv2.row(r)), 2.0 * dv1.row(r).dot(dv3.row(r)),
          2.0 * dv2.row(r).dot(dv3.row(r)), dv3.row(r).dot(dv3.row(r)), 2.0 * dv1.row(r).dot(dv4.row(r)), 2.0 * dv2.row(r).dot(dv4.row(r)),
          2.0 * dv3.row(r).dot(dv4.row(r)), dv4.row(r).dot(dv4.row(r));
    }

    Vec rho;
    rho.resize(6);
    rho << (xControlPoints.col(0) - xControlPoints.col(1)).squaredNorm(), (xControlPoints.col(0) - xControlPoints.col(2)).squaredNorm(),
      (xControlPoints.col(0) - xControlPoints.col(3)).squaredNorm(), (xControlPoints.col(1) - xControlPoints.col(2)).squaredNorm(),
      (xControlPoints.col(1) - xControlPoints.col(3)).squaredNorm(), (xControlPoints.col(2) - xControlPoints.col(3)).squaredNorm();
