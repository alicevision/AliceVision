...
{
    return p(coef_xxx) * x * x * x + p(coef_xxy) * x * x * y + p(coef_xxz) * x * x * z + p(coef_xyy) * x * y * y +
           p(coef_xyz) * x * y * z + p(coef_xzz) * x * z * z + p(coef_yyy) * y * y * y + p(coef_yyz) * y * y * z +
           p(coef_yzz) * y * z * z + p(coef_zzz) * z * z * z + p(coef_xx) * x * x + p(coef_xy) * x * y +
           p(coef_xz) * x * z + p(coef_yy) * y * y + p(coef_yz) * y * z + p(coef_zz) * z * z + p(coef_x) * x +
           p(coef_y) * y + p(coef_z) * z + p(coef_1) * 1;
}
...
BOOST_AUTO_TEST_CASE(o1_Evaluation)
{
    ...
    for(double z = -5; z < 5; ++z)
    {
        for(double y = -5; y < 5; ++y)
        {
            ...
            {
                BOOST_CHECK_SMALL(
                    EvalPolynomial(p3, x, y, z) - EvalPolynomial(p1, x, y, z) * EvalPolynomial(p2, x, y, z), 1e-8);
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE(FivePointsRelativePose_Random)
{
    ...
    for(size_t s = 0; s < Es.size(); ++s)
    {
        ...
        BOOST_CHECK(MotionFromEssentialAndCorrespondence(Es[s], Mat3::Identity(), x1Col, Mat3::Identity(), x2Col,
                                                         &Rs[s], &ts[s]));
    }
...
BOOST_AUTO_TEST_CASE(FivePointsRelativePose_test_data_sets)
{
    ...
    const int iNviews = 5;
    NViewDataSet d = NRealisticCamerasRing(
        iNviews, 5, NViewDatasetConfigurator(1, 1, 0, 0, 5, 0)); // Suppose a camera with Unit matrix as K
    ...
    for(int i = 1; i < iNviews; ++i)
    {
        ...
        for(size_t s = 0; s < Es.size(); ++s)
        {
            ...
            BOOST_CHECK(MotionFromEssentialAndCorrespondence(Es[s], d._K[0], x1Col, d._K[i], x2Col, &Rs[s], &ts[s]));
        }
