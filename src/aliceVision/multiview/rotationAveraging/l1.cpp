...
inline bool TRobustRegressionL1PD(const MATRIX_TYPE& A,
...
bool RobustRegressionL1PD(const Eigen::Matrix<REAL, Eigen::Dynamic, Eigen::Dynamic>& A,
...
bool RobustRegressionL1PD(const Eigen::SparseMatrix<REAL, Eigen::ColMajor>& A,
                          const Eigen::Matrix<REAL, Eigen::Dynamic, 1>& b,
                          Eigen::Matrix<REAL, Eigen::Dynamic, 1>& x,
                          REAL pdtol,
                          unsigned pdmaxiter)
...
template<typename MATRIX_TYPE>
inline bool TIterativelyReweightedLeastSquares(const MATRIX_TYPE& A,
                                               const Eigen::Matrix<REAL, Eigen::Dynamic, 1>& b,
                                               Eigen::Matrix<REAL, Eigen::Dynamic, 1>& x,
                                               REAL sigma,
                                               REAL eps)
...
bool IterativelyReweightedLeastSquares(const Eigen::Matrix<REAL, Eigen::Dynamic, Eigen::Dynamic>& A,
                                       const Eigen::Matrix<REAL, Eigen::Dynamic, 1>& b,
                                       Eigen::Matrix<REAL, Eigen::Dynamic, 1>& x,
                                       REAL sigma,
                                       REAL eps)
...
bool IterativelyReweightedLeastSquares(const Eigen::SparseMatrix<REAL, Eigen::ColMajor>& A,
                                       const Eigen::Matrix<REAL, Eigen::Dynamic, 1>& b,
                                       Eigen::Matrix<REAL, Eigen::Dynamic, 1>& x,
                                       REAL sigma,
                                       REAL eps)
...
inline std::pair<TYPE, TYPE> ComputeX84Threshold(const TYPE* const values, size_t size, TYPE mul = TYPE(5.2))
{
    assert(size > 0);
...
struct Link
{
    ...
    inline Link(size_t _ID = 0, size_t _parentID = 0)
      : ID(_ID),
        parentID(_parentID)
    {}
};
...
size_t FindMaximumSpanningTree(const RelativeRotations& RelRs, graph_t& g, MapEdgeIJ2R& mapIJ2R, NodeArr& minGraph)
{
    ...
    for (std::set<size_t>::const_iterator iter = setNodes.begin(); iter != setNodes.end(); ++iter)
    {
...
unsigned int FilterRelativeRotations(const RelativeRotations& RelRs, const Matrix3x3Arr& Rs, float threshold, std::vector<bool>* vec_inliers)
{
...
#ifdef ALICEVISION_ROTATION_AVERAGING_WITH_BOOST
    boost::accumulators::
      accumulator_set<REAL, boost::accumulators::stats<boost::accumulators::tag::min, boost::accumulators::tag::mean, boost::accumulators::tag::max>>
        acc;
...
void InitRotationsMST(const RelativeRotations& RelRs, Matrix3x3Arr& Rs, const size_t nMainViewID)
{
    assert(!Rs.empty());
    ...
    do
    {
        ...
        for (Node::InternalType::const_iterator pEdge = node.edges.begin(); pEdge != node.edges.end(); ++pEdge)
        {
...
bool GlobalRotationsRobust(const RelativeRotations& RelRs,
                           Matrix3x3Arr& Rs,
...
// build A in Ax=b
inline void _FillMappingMatrix(const RelativeRotations& RelRs, const size_t nMainViewID, Eigen::SparseMatrix<REAL, Eigen::ColMajor>& A)
{
...
// compute errors for each relative rotation
inline void _FillErrorMatrix(const RelativeRotations& RelRs, const Matrix3x3Arr& Rs, Eigen::Matrix<REAL, Eigen::Dynamic, 1>& b)
{
...
inline void _CorrectMatrix(const Eigen::Matrix<REAL, Eigen::Dynamic, 1>& x, const size_t nMainViewID, Matrix3x3Arr& Rs)
{
...
bool RefineRotationsAvgL1IRLS(const RelativeRotations& RelRs, Matrix3x3Arr& Rs, const size_t nMainViewID, REAL sigma)
{
    assert(!RelRs.empty() && !Rs.empty());
    ...
    ALICEVISION_LOG_DEBUG("Refine global rotations using L1RA-IRLS and "
                          << nObss << " relative rotations:\n"
