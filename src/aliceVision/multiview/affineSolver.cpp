...
//                      | x |
//                      | y |
bool affine2DFromCorrespondencesLinear(const Mat& x1, const Mat& x2, Mat3* M, double expected_precision)
{
    assert(2 == x1.rows());
    assert(3 <= x1.cols());
    ...
    {
        *M << x(0), x(1), x(4), x(2), x(3), x(5), 0.0, 0.0, 1.0;
        return true;
    }
    else
    {
...
//                                      | y |
//                                      | z |
bool affine3DFromCorrespondencesLinear(const Mat& x1, const Mat& x2, Mat4* M, double expected_precision)
{
    assert(3 == x1.rows());
    assert(4 <= x1.cols());
    assert(x1.rows() == x2.rows());
