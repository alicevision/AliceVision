...
void visibleCamPosToSVGSurface(const std::vector<Vec3>& vec_Ci, const std::string& fileName)
{
    ...
    if (!fileName.empty())
    {
        ...
        for (size_t i = 0; i < vec_Ci.size(); ++i)
        {
            svgSurface_GT.drawCircle(out[i](0), out[i](2), 3, svgStyle().stroke("black", 0.2).fill("red"));
        }
...
NViewDataSet Setup_RelativeTranslations_AndNviewDataset(std::vector<aliceVision::translationAveraging::relativeInfo>& vec_relative_estimates,
                                                        ...
                                                        const bool bRelative_Translation_PerTriplet = false)
{
    const NViewDataSet d =
      bCardiod ? NRealisticCamerasCardioid(iNviews, iNbPoints, NViewDatasetConfigurator(focal, focal, principal_Point, principal_Point, 5, 0))
               : NRealisticCamerasRing(iNviews, iNbPoints, NViewDatasetConfigurator(focal, focal, principal_Point, principal_Point, 5, 0));
    ...
    for (const std::pair<size_t, size_t>& iter : map_pairs)
    {
        ...
        //-- Build relative motions (that feeds the Linear program formulation)
        {
            ...
            vec_relative_estimates.push_back(std::make_pair(std::make_pair(I, J), std::make_pair(RijGt, tij)));
        }
    }
    return d;
}
