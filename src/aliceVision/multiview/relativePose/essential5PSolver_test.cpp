...
double EvalPolynomial(Vec p, double x, double y, double z)
{
    ...
    return p(Pc::coef_xxx) * x * x * x + p(Pc::coef_xxy) * x * x * y + p(Pc::coef_xxz) * x * x * z + p(Pc::coef_xyy) * x * y * y +
           p(Pc::coef_xyz) * x * y * z + p(Pc::coef_xzz) * x * z * z + p(Pc::coef_yyy) * y * y * y + p(Pc::coef_yyz) * y * y * z +
           p(Pc::coef_yzz) * y * z * z + p(Pc::coef_zzz) * z * z * z + p(Pc::coef_xx) * x * x + p(Pc::coef_xy) * x * y + p(Pc::coef_xz) * x * z +
           p(Pc::coef_yy) * y * y + p(Pc::coef_yz) * y * z + p(Pc::coef_zz) * z * z + p(Pc::coef_x) * x + p(Pc::coef_y) * y + p(Pc::coef_z) * z +
           p(Pc::coef_1) * 1;
}
...
BOOST_AUTO_TEST_CASE(FivePointsRelativePose_Random)
{
    for (std::size_t trial = 0; trial < 100; ++trial)
    {
        ...
        for (std::size_t s = 0; s < Es.size(); ++s)
        {
            ...
            {
                const double v = d.x2.col(c).homogeneous().transpose() * Es.at(s).getMatrix() * d.x1.col(c).homogeneous();
                BOOST_CHECK_SMALL(v, 1.0e-6);
            }

            Vec2 x1Col = d.x1.col(0);
            Vec2 x2Col = d.x2.col(0);
            BOOST_CHECK(motionFromEssentialAndCorrespondence(Es.at(s).getMatrix(), Mat3::Identity(), x1Col, Mat3::Identity(), x2Col, &Rs[s], &ts[s]));
        }
...
BOOST_AUTO_TEST_CASE(FivePointsRelativePose_test_data_sets)
{
    ...
    const int iNviews = 5;
    NViewDataSet d = NRealisticCamerasRing(iNviews, 5, NViewDatasetConfigurator(1, 1, 0, 0, 5, 0));  // Suppose a camera with Unit matrix as K
    ...
    for (int i = 1; i < iNviews; ++i)
    {
        ...
        for (size_t s = 0; s < Es.size(); ++s)
        {
            ...
            BOOST_CHECK(motionFromEssentialAndCorrespondence(Es.at(s).getMatrix(), d._K[0], x1Col, d._K[i], x2Col, &Rs[s], &ts[s]));
        }
        ...
        for (size_t nModel = 0; nModel < Es.size(); ++nModel)
        {
            ...
            // Check that we find the correct relative orientation.
            if (FrobeniusDistance(R, Rs[nModel]) < 1e-3 && (t / t.norm() - ts[nModel] / ts[nModel].norm()).norm() < 1e-3)
            {
                bsolution_found = true;
            }
        }
