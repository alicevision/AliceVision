...
NViewDatasetConfigurator::NViewDatasetConfigurator(int fx, int fy, int cx, int cy, double distance, double jitter_amount)
  : _fx(fx),
    _fy(fy),
    _cx(cx),
    _cy(cy),
...
void NViewDataSet::exportToPLY(const std::string& out_file_name) const
{
    ...
    if (outfile.is_open())
    {
        outfile << "ply" << std::endl
                ...
                << "comment NViewDataSet export" << std::endl
                << "comment It shows 3D point structure and cameras"
                << "+ camera looking direction" << std::endl
                << "element vertex " << _X.cols() + _t.size() * 2 << std::endl
        ...
        for (size_t i = 0; i < _t.size(); ++i)
        {
            ...
            outfile << ((-_R[i].transpose() * _t[i]) + (_R[i].transpose() * test)).transpose() << " "
                    << "255 0 0" << std::endl;
        }
        outfile.close();
    }
...
NViewDataSet NRealisticCamerasCardioid(size_t nviews, size_t npoints, const NViewDatasetConfigurator& config)
{
    ...
    for (size_t i = 0; i < nviews; ++i)
    {
        ...
        const double theta = i * 2 * M_PI / nviews;
        //-- Cardioid
        camera_center << 2 * sin(theta) - (sin(2 * theta)), 0.0,
