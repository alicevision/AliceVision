...
BOOST_AUTO_TEST_CASE(Translation_Structure_L_Infinity_Noisy_Outlier_OSICLP_SOLVER)
{
    ...
    const NViewDataSet d =
      ...
      NRealisticCamerasCardioid(nViews, nbPoints, NViewDatasetConfigurator(focalValue, focalValue, cx, cy, 5, 0));
...

    {
        ...
        Mat3 K;
        K << focalValue, 0, cx, 0, focalValue, cy, 0, 0, 1;
        ...
        const bool bisectionRes = BisectionLP<TiXi_withNoise_L1_ConstraintBuilder, LPConstraintsSparse>(
          wrapperOSICLPSolver, cstBuilder, &vec_solution, admissibleResidual, 0.0, 1e-8);
