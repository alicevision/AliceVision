...
{
    NViewDataSet d = NRealisticCamerasRing(6, 10, NViewDatasetConfigurator(1, 1, 0, 0, 5, 0));  // Suppose a camera with Unit matrix as K
    ...
    for (int k = 0; k < d._x[0].cols(); ++k)
    {
        ...
        BOOST_CHECK((BisectionLP<Triangulation_L1_ConstraintBuilder, LPConstraints>(wrapperOSICLPSolver, cstBuilder, &vec_solution, 1.0, 0.0)));
...
BOOST_AUTO_TEST_CASE(computervision_Triangulation_MOSEK)
{
    ...
    for (int k = 0; k < d._x[0].cols(); ++k)
    {
        ...
        // Use bisection in order to find the global optimum and so find the
        //  best triangulated point under the L_infinity norm
        BOOST_CHECK((BisectionLP<Triangulation_L1_ConstraintBuilder, LPConstraints>(wrapperLpSolve, cstBuilder, &vec_solution, 1.0, 0.0)));
