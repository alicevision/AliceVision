...
void centerMatrix(const Eigen::Matrix2Xf& points2d, Mat3& t)
{
    ...
    t << 1. / stdDev(0), 0., -mean(0) / stdDev(0), 0., 1. / stdDev(1), -mean(1) / stdDev(1), 0., 0., 1.;
}
...
void computeSimilarity(const feature::PointFeature& feat1, const feature::PointFeature& feat2, Mat3& S)
{
    S = Mat3::Identity();
    ...
    Mat3 A1;
    A1 << scale1 * c1, scale1 * (-s1), coord1(0), scale1 * s1, scale1 * c1, coord1(1), 0, 0, 1;
    Mat3 A2;
    A2 << scale2 * c2, scale2 * (-s2), coord2(0), scale2 * s2, scale2 * c2, coord2(1), 0, 0, 1;
 ...
 * @details It is based on [F.Srajer, 2016] p.20, 21 and its C++ implementation:
 * https://github.com/fsrajer/yasfm/blob/master/YASFM/relative_pose.cpp#L992 "The optimization takes into account points with error close to the
 * threshold and does not care about high-error ones."
 */
class RefineHRobustCostFunctor
{
  public:
    RefineHRobustCostFunctor(const Vec2& x1, const Vec2& x2, double softThresh)
      : x1(x1),
        x2(x2),
        _softThresh(softThresh)
    {}
                      ...
                      double homographyTolerance)
{
    ...
    return refineHomography(pointsI, pointsJ, remainingMatches, homography, bestMatchesId, homographyTolerance);
}
                      ...
                      double homographyTolerance)
{
    ...
    for (IndexT matchId : bestMatchesId)
    {
        ...
        RefineHRobustCostFunctor* costFun = new RefineHRobustCostFunctor(x1, x2, homographyTolerance);

        problem.AddResidualBlock(new ceres::AutoDiffCostFunction<RefineHRobustCostFunctor, 1, 9>(costFun), nullptr, tempHomography.data());
    }
