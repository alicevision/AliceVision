...
BOOST_AUTO_TEST_CASE(read_write_grayscale)
{
    ...
    for (const auto& extension : extensions)
    {
        ...
        BOOST_CHECK_NO_THROW(writeImage(filename, image, image::ImageWriteOptions().toColorSpace(image::EImageColorSpace::NO_CONVERSION)));
...
BOOST_AUTO_TEST_CASE(read_write_rgba)
{
    ...
    for (const auto& extension : extensions)
    {
        if (extension == "jpg" || extension == "pgm" || extension == "ppm")
...
BOOST_AUTO_TEST_CASE(read_write_from_rgb_to_grayscale)
{
    ...
    for (const auto& extension : extensions)
    {
        ...
        BOOST_CHECK_NO_THROW(writeImage(filename, imageRGB, image::ImageWriteOptions().toColorSpace(image::EImageColorSpace::NO_CONVERSION)));
...
BOOST_AUTO_TEST_CASE(read_write_from_grayscale_to_rgb)
{
    ...
    for (const auto& extension : extensions)
    {
        ...
        BOOST_CHECK_NO_THROW(writeImage(filename, imageGrayscale, image::ImageWriteOptions().toColorSpace(image::EImageColorSpace::NO_CONVERSION)));
