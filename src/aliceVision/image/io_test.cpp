...
BOOST_AUTO_TEST_CASE(read_unexisting) {
  ...
  BOOST_CHECK_THROW(readImage(fs, filename, image, image::EImageColorSpace::NO_CONVERSION), std::exception);
}
...
BOOST_AUTO_TEST_CASE(read_jpg_grayscale) {
  ...
  BOOST_CHECK_NO_THROW(readImage(fs, jpg_filename, image, image::EImageColorSpace::NO_CONVERSION));
...
BOOST_AUTO_TEST_CASE(read_pgm) {
  ...
  BOOST_CHECK_NO_THROW(readImage(fs, pgm_filename, image, image::EImageColorSpace::NO_CONVERSION));
  BOOST_CHECK_EQUAL(2, image.Width());
  BOOST_CHECK_EQUAL(1, image.Height());
  BOOST_CHECK_EQUAL(1, image.Depth());
  BOOST_CHECK_EQUAL(image(0,0), (unsigned char)255);
...
BOOST_AUTO_TEST_CASE(read_ppm) {
  ...
  BOOST_CHECK_NO_THROW(readImage(fs, ppm_filename, image, image::EImageColorSpace::NO_CONVERSION));
  BOOST_CHECK_EQUAL(2, image.Width());
  BOOST_CHECK_EQUAL(1, image.Height());
  BOOST_CHECK_EQUAL(3, image.Depth());
...
BOOST_AUTO_TEST_CASE(read_write_grayscale) {
  ...
  for(const auto& extension : extensions)
  {
    ...
    BOOST_CHECK_NO_THROW(writeImage(fs, filename, image, image::EImageColorSpace::NO_CONVERSION));

    Image<unsigned char> read_image;
    BOOST_CHECK_NO_THROW(readImage(fs, filename, read_image, image::EImageColorSpace::NO_CONVERSION));
...
BOOST_AUTO_TEST_CASE(read_write_from_rgb_to_grayscale) {
  ...
  for(const auto& extension : extensions)
  {
    ...
    BOOST_CHECK_NO_THROW(writeImage(fs, filename, imageRGB, image::EImageColorSpace::NO_CONVERSION));
...
BOOST_AUTO_TEST_CASE(read_write_from_grayscale_to_rgb) {
  ...
  for(const auto& extension : extensions)
  {
    ...
    BOOST_CHECK_NO_THROW(writeImage(fs, filename, imageGrayscale, image::EImageColorSpace::NO_CONVERSION));
