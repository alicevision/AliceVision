...
const std::vector<geometry::Pose3>& Rig::getRelativePoses() const { return _vRelativePoses; }
...
void Rig::setTrackingResult(std::vector<localization::LocalizationResult> vLocalizationResults, std::size_t i)
{
    ...
    for (std::size_t iLocalizer = 1; iLocalizer < nCams; ++iLocalizer)
    {
        ...
        for (std::size_t iView = 0; iView < resWitnessCamera.size(); ++iView)
        {
            ...
            if (resMainCamera[iView].isValid() && resWitnessCamera[iView].isValid())
            {
                ...
                vRelativePoses.push_back(computeRelativePose(resMainCamera[iView].getPose(), resWitnessCamera[iView].getPose()));
            }
        }
    ...
    for (std::size_t iRelativePose = 0; iRelativePose < _vRelativePoses.size(); ++iRelativePose)
    {
        ...
        for (std::size_t iView = 0; iView < _vLocalizationResults[iRes].size(); ++iView)
        {
            if (_vLocalizationResults[0][iView].isValid())
            {
                ...
                _vLocalizationResults[iRes][iView].applyPose(poseWitnessCamera);
            }
        }
    }
    _isInitialized = true;
...
void Rig::findBestRelativePose(const std::vector<geometry::Pose3>& vPoses, std::size_t iLocalizer, geometry::Pose3& result)
{
...
bool Rig::optimizeCalibration()
{
            ...
            for (const IndexT iPoint : currentResult[iView].getInliers())
            {
                ...
                if (iLocalizer == 0)
                {
                    ...
                    if (cost_function)
                    {
                        ...
                        problem.AddResidualBlock(cost_function, p_LossFunction, &vMainPoses[iView][0]);
                    }
                    else
                    {
                ...
                // Add a residual block for a secondary camera
                {
                    ...
                    if (cost_function)
                    {
                        ...
                        problem.AddResidualBlock(cost_function, p_LossFunction, &vMainPoses[iView][0], &vRelativePoses[iLocalizer - 1][0]);
                    }
                    else
                    {
    ...
    for (std::size_t iView = 0; iView < _vLocalizationResults[0].size(); ++iView)
    {
        if (_vLocalizationResults[0][iView].isValid())
        {
            ...
            _vLocalizationResults[0][iView].applyPose(pose);
    ...
    for (std::size_t iRelativePose = 0; iRelativePose < _vRelativePoses.size(); ++iRelativePose)
    {
        ...
        for (std::size_t iView = 0; iView < _vLocalizationResults[iLocalizer].size(); ++iView)
        {
            ...
            if (_vLocalizationResults[0][iView].isValid() && _vLocalizationResults[iLocalizer][iView].isValid())
            {
                ...
                const geometry::Pose3 poseWitnessCamera =
                  poseFromMainToWitness(_vLocalizationResults[0][iView].getPose(), _vRelativePoses[iRelativePose]);
                _vLocalizationResults[iLocalizer][iView].applyPose(poseWitnessCamera);
            }
        }
...
bool Rig::saveCalibration(const std::string& filename) { return saveRigCalibration(filename, _vRelativePoses); }
