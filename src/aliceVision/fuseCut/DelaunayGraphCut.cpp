                                    ...
                                    std::vector<GC_vertexInfo>& verticesAttrPrepare, mvsUtils::MultiViewParams& mp, float simFactor, float voteMarginFactor, float contributeMarginFactor, float simGaussianSize)
{
    ...
    for(int c = 0; c < cams.size(); ++c)
    {
        ...
        int width, height;
        {
            ...
            imageIO::readImage(fs, depthMapFilepath, width, height, depthMap, imageIO::EImageColorSpace::NO_CONVERSION);
            if(depthMap.empty())
            {
            ...
            {
                imageIO::readImage(fs, simMapFilepath, wTmp, hTmp, simMap, imageIO::EImageColorSpace::NO_CONVERSION);
    ...
    ALICEVISION_LOG_INFO("Load depth maps and add points.");
    {
        for(int c = 0; c < cams.size(); c++)
        {
            ...
            int width, height;
            {
                ...
                imageIO::readImage(_fs, depthMapFilepath, width, height, depthMap, imageIO::EImageColorSpace::NO_CONVERSION);
                if(depthMap.empty())
                {
                    ALICEVISION_LOG_WARNING("Empty depth map: " << depthMapFilepath);
                    continue;
    ...
    ALICEVISION_LOG_INFO("Load depth maps and add points.");
    {
        ...
        for(int c = 0; c < cams.size(); c++)
        {
            ...
            int width, height;
            {
                ...
                {
                    imageIO::readImage(_fs, simMapFilepath, wTmp, hTmp, simMap, imageIO::EImageColorSpace::NO_CONVERSION);
                    if(wTmp != width || hTmp != height)
                ...
                {
                    imageIO::readImage(_fs, nmodMapFilepath, wTmp, hTmp, numOfModalsMap,
...
void DelaunayGraphCut::voteFullEmptyScore(const StaticVector<int>& cams, const std::string& folderName)
{
    ...
    if(jancosekIJCV) // true by default
    {
        ...
        if(false)
        {
            ...
            meshf->save(_fs, folderName + "tetrahedralMesh_beforeForceTEdge_emptiness");
        }
...
void DelaunayGraphCut::exportDebugMesh(const std::string& filename, const Point3d& fromPt, const Point3d& toPt)
{
    ...
    mesh->save(_fs, tempDirPath + "/" + filename);
    meshf->save(_fs, tempDirPath + "/" + filename);
}
...
void DelaunayGraphCut::exportFullScoreMeshs(const std::string& outputFolder, const std::string& name) const
{
    ...
    }
    {
        ...
        mesh->save(_fs, outputFolder + "/mesh_fullness-emptiness" + nameExt);
    }
    {
        std::unique_ptr<mesh::Mesh> mesh(createTetrahedralMesh(
            false, 0.99f, [](const fuseCut::GC_cellInfo& c) { return c.cellSWeight - c.cellTWeight; }));
        mesh->save(_fs, outputFolder + "/mesh_s-t" + nameExt);
    }
}
