...
BOOST_AUTO_TEST_CASE(LOCAL_BUNDLE_ADJUSTMENT_EffectiveMinimization_Pinhole_CamerasRing)
{
  ...
  // Check views:
  BOOST_CHECK( !(sfmData.getPose(*sfmData.getViews().at(0).get()) == sfmData_notRefined.getPose(*sfmData_notRefined.getViews().at(0).get())) ); // v0 refined
  BOOST_CHECK( !(sfmData.getPose(*sfmData.getViews().at(1).get()) == sfmData_notRefined.getPose(*sfmData_notRefined.getViews().at(1).get())) ); // v1 refined
  BOOST_CHECK( sfmData.getPose(*sfmData.getViews().at(2).get()) == sfmData_notRefined.getPose(*sfmData_notRefined.getViews().at(2).get()) ); // v2 constant
...
SfMData getInputScene(const NViewDataSet & d, const NViewDatasetConfigurator & config, EINTRINSIC eintrinsic)
{
  ...
  // 3. Intrinsic data (shared, so only one camera intrinsic is defined)
  {
    ...
    sfm_data.intrinsics.emplace(0, createIntrinsic(eintrinsic, w, h, config._fx, config._fx, 0, 0));
  }
