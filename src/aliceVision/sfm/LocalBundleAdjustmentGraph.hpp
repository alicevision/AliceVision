...
class LocalBundleAdjustmentGraph
{
  public:
     ...
     */
    inline std::size_t getGraphDistanceLimit() const { return _graphDistanceLimit; }

    /**
     * @brief Set the graph-distance limit setting the Active region
     * @param[in] graph-distance limit setting
     */
    inline void setGraphDistanceLimit(const std::size_t limit) { _graphDistanceLimit = limit; }
  ...
  private:
     ...
     * @param[in] stdevPercentageLimit The limit is reached when the standard deviation of the \a windowSize values is less than \a
     * stdevPecentageLimit % of the range of all the values.
     */
     ...
     */
    inline bool isFocalLengthConstant(const IndexT intrinsicId) const { return _mapFocalIsConstant.at(intrinsicId); }
