...
LocalBundleAdjustmentGraph::LocalBundleAdjustmentGraph(const sfmData::SfMData& sfmData)
{
    ...
    //Group views by rig/pose
    std::map<IndexT, std::map<IndexT, std::set<IndexT>>> rigViewsGroupedByTime;
    for (const auto & v : sfmData.getViews())
    {
        IndexT poseId = v.second->getPoseId();
        IndexT rigId = v.second->getRigId();

        if (rigId != UndefinedIndexT && poseId != UndefinedIndexT)
        {
            rigViewsGroupedByTime[rigId][poseId].insert(v.first);
        }
    }

    //Compute neighboors of a view inside a rig at a given time
    for (auto r : rigViewsGroupedByTime)
    {
        for (auto p : r.second)
        {
            for (auto idr : p.second)
            {
                std::set<IndexT> neighboors;
                for (auto idc : p.second)
                {
                    if (idc == idr)
                    {
                        continue;
                    ...
                    neighboors.insert(idc);
                }

                _rigLinksPerView[idr] = neighboors;
            }
        }
    }
}
...
void LocalBundleAdjustmentGraph::exportIntrinsicsHistory(const std::string& folder, const std::string& filename)
{
    ...
    for(std::size_t i = 0; i < nbIterations; ++i)
    {
        for(const auto& intrinsicHistoryPair : _intrinsicsHistory)
        {
            ...
            os << intrinsicsHistory.nbPoses << ";" << intrinsicsHistory.focalLength << ";"
               << intrinsicsHistory.isConstant << ";;";
        }
        os << "\n";
    }
    os.close();
...
bool LocalBundleAdjustmentGraph::removeViews(const sfmData::SfMData& sfmData, const std::set<IndexT>& removedViewsId)
{
    ...
    for(auto& edgesIt : removedEdgesByIntrinsic)
    {
        ...
        std::set_difference(edgeIds.begin(), edgeIds.end(), removedEdges.begin(), removedEdges.end(), std::back_inserter(newEdgeIds));
...
int LocalBundleAdjustmentGraph::getPoseDistance(const IndexT poseId) const
{
    ...
    {
        ALICEVISION_LOG_DEBUG("The pose #" << poseId << " does not exist in the '_mapDistancePerPoseId' (map size: "
                                           << _distancePerPoseId.size() << ") \n");
        return -1;
    }
...
int LocalBundleAdjustmentGraph::getViewDistance(const IndexT viewId) const
{
    ...
    {
        ALICEVISION_LOG_DEBUG("Cannot get the graph-distance of the view #"
                              << viewId << ": does not exist in the '_mapDistancePerViewId':\n");
        return -1;
    }
                                                         ...
                                                         const std::size_t minNbOfMatches)
{
    ...
    if(!addedViewsId.empty())
    {
        ...
        // we create the 'minNbOfEdgesPerView' best edges and all the other with more than 'minNbOfMatches' shared
        // landmarks
    ...
    for(const IndexT& viewId : addedViewsId)
    {
        const auto & neighboors = _rigLinksPerView[viewId];
        ...
        {
            if (_nodePerViewId.find(nb) == _nodePerViewId.end())
            {
                continue;
            }

            _graph.addEdge(_nodePerViewId.at(viewId), _nodePerViewId.at(nb));
            numAddedEdges++;
        }
    }
...
void LocalBundleAdjustmentGraph::computeGraphDistances(const sfmData::SfMData& sfmData,
                                                       const std::set<IndexT>& newReconstructedViews)
{
    ...
    for(const IndexT viewId : newReconstructedViews)
    {
        ...
        if(it == _nodePerViewId.end())
            ALICEVISION_LOG_WARNING("The reconstructed view #"
                                    << viewId
                                    << " cannot be added as source for the BFS: does not exist in the graph.");
        else
...
void LocalBundleAdjustmentGraph::convertDistancesToStates(const sfmData::SfMData& sfmData)
{
    ...
    const double kStdevPercentage =
        1.0; //< limit percentage of the Std deviation according to the range of all the parameters (e.i. focal)
...
std::vector<Pair> LocalBundleAdjustmentGraph::getNewEdges(const sfmData::SfMData& sfmData,
                                                          ...
                                                          const std::size_t minNbOfEdgesPerView)
{
    ...
    std::transform(sfmData.getLandmarks().begin(), sfmData.getLandmarks().end(),
                   std::inserter(landmarkIds, landmarkIds.begin()), stl::RetrieveKey());
    ...
    for(IndexT viewId : newViewsId)
    {
        ...
        std::set_intersection(newViewTrackIds.begin(), newViewTrackIds.end(), landmarkIds.begin(), landmarkIds.end(),
        ...
        std::sort(sharedLandmarksPerViewSorted.begin(), sharedLandmarksPerViewSorted.end(),
                  [](const ViewNbLandmarks& a, const ViewNbLandmarks& b) { return (a.second > b.second); });

        std::size_t nbEdgesPerView = 0;
        for(const ViewNbLandmarks& sharedLandmarkPair : sharedLandmarksPerViewSorted)
        {
            if(nbEdgesPerView >= minNbOfEdgesPerView && sharedLandmarkPair.second < minNbOfMatches)
                break;

            // edges format: pair<min_viewid, max_viewid>
            newEdges.emplace_back(std::min(viewId, sharedLandmarkPair.first),
                                  std::max(viewId, sharedLandmarkPair.first));
            ++nbEdgesPerView;
        }
    }
    return newEdges;
...
void LocalBundleAdjustmentGraph::checkFocalLengthsConsistency(const std::size_t windowSize,
...
void LocalBundleAdjustmentGraph::drawGraph(const sfmData::SfMData& sfmData, const std::string& folder,
                                           const std::string& nameComplement)
{
    if(!fs::exists(folder))
    ...
    for(lemon::ListGraph::NodeIt n(_graph); n != lemon::INVALID; ++n)
    {
        ...
        if(viewDist == 0)
            ...
            color += "black";
        dotStream << "  n" << _graph.id(n) << " [ label=\"" << viewId << ": D" << viewDist << " K"
                  << sfmData.getViews().at(viewId)->getIntrinsicId() << "\"" << color << "]; "
                  << "\n";
    }
    ...
    {
        dotStream << "  n" << _graph.id(_graph.u(e)) << " -> "
                  << " n" << _graph.id(_graph.v(e));
    ...
    const std::string dotFilepath =
        (fs::path(folder) / ("graph_" + std::to_string(_viewIdPerNode.size()) + "_" + nameComplement + ".dot"))
            .string();
...
std::size_t LocalBundleAdjustmentGraph::addIntrinsicEdgesToTheGraph(const sfmData::SfMData& sfmData,
                                                                    const std::set<IndexT>& newReconstructedViews)
{
    ...
    for(IndexT newViewId : newReconstructedViews) // for each new view
    {
        ...
        for(const auto& x : _nodePerViewId)
        {
            ...
            // note: do not compare a view with itself (must be tested since "_nodePerViewId" contains
            // "newReconstructedViews")
            if(newViewId != otherViewId && newViewIntrinsicId == sfmData.getViews().at(otherViewId)->getIntrinsicId())
            {
    ...
    for(const auto& newEdge : newIntrinsicEdges)
    {
        const lemon::ListGraph::Edge edge =
            _graph.addEdge(_nodePerViewId[newEdge.first.first], _nodePerViewId[newEdge.first.second]);
