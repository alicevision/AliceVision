                                              ...
                                              matching::PairwiseMatches& tripletWise_matches)
{
    ...
    Compute_translations(sfmData, normalizedFeaturesPerView, pairwiseMatches, map_globalR, randomNumberGenerator, tripletWise_matches);
                                                                ...
                                                                const HashMap<IndexT, Mat3>& map_globalR)
{
    ...
    const std::set<IndexT> set_remainingIds = aliceVision::graph::CleanGraph_KeepLargestBiEdge_Nodes<PairSet, IndexT>(pairs, "./");
...

    {
        ...
        switch (eTranslationAveragingMethod)
        {
            case TRANSLATION_AVERAGING_L1:
            {
                ...
                std::vector<double> vec_camRelLambdas(&vec_solution[iNview * 3],
                                                      &vec_solution[iNview * 3 + vec_initialRijTijEstimates_cpy.size() / 3]);
                ...
                for (std::size_t i = 0; i < iNview; ++i)
                {
                    ...
                    sfmData.createPose(pose_id, CameraPose(Pose3(Ri, -Ri.transpose() * t)));
                }
            }
            break;
            ...
            case TRANSLATION_AVERAGING_SOFTL1:
            {
                ...
                if (!translationAveraging::solve_translations_problem_softl1(vec_initialRijTijEstimates_cpy, true, iNview, vec_translations))
                {
                    ALICEVISION_LOG_WARNING("Compute global translations: failed");
                    return false;
                }
            ...
            case TRANSLATION_AVERAGING_L2_DISTANCE_CHORDAL:
            {
                ...
                if (!translationAveraging::solve_translations_problem_l2_chordal(&vec_edges[0],
                                                                                 &vec_poses[0],
                                                                                 &vec_weights[0],
                ...
                for (std::size_t i = 0; i < iNview; ++i)
                {
                    ...
                    const Mat3& Ri = map_globalR.at(pose_id);
                    sfmData.createPose(pose_id, CameraPose(Pose3(Ri, C)));
                }
            }
            break;
            default:
                                                               ...
                                                               matching::PairwiseMatches& tripletWise_matches)
{
    ...
    ComputePutativeTranslation_EdgesCoverage(
      sfmData, map_globalR, normalizedFeaturesPerView, pairwiseMatches, randomNumberGenerator, m_vec_initialRijTijEstimates, tripletWise_matches);
}
                                                                                   ...
                                                                                   matching::PairwiseMatches& newpairMatches)
{
    ...
    std::transform(map_globalR.begin(), map_globalR.end(), std::inserter(set_pose_ids, set_pose_ids.begin()), stl::RetrieveKey());
    ...
    for (const auto& match_iterator : pairwiseMatches)
    {
        ...
        if (  // Consider the pair iff it is supported by the rotation graph
          (v1->getPoseId() != v2->getPoseId()) && set_pose_ids.count(v1->getPoseId()) && set_pose_ids.count(v2->getPoseId()))
        {
            rotation_pose_id_graph.insert(std::make_pair(v1->getPoseId(), v2->getPoseId()));
        }
    }
    // List putative triplets (from global rotations Ids)
    const std::vector<graph::Triplet> vec_triplets = graph::tripletListing(rotation_pose_id_graph);
        ...
        for (int i = 0; i < (int)vec_triplets.size(); ++i)
        {
            ...
            for (const auto& match_iterator : pairwiseMatches)
            {
                ...
                if (  // Consider the pair iff it is supported by the triplet graph & 2 different pose id
                  (v1->getPoseId() != v2->getPoseId()) && set_triplet_pose_ids.count(v1->getPoseId()) && set_triplet_pose_ids.count(v2->getPoseId()))
                {
...
#pragma omp critical
        ...
        auto progressDisplay =
          system::createConsoleProgressDisplay(vec_edges.size(), std::cout, "\nRelative translations computation (edge coverage algorithm)\n");
        ...
        for (int k = 0; k < vec_edges.size(); ++k)
        {
            ...
            if (m_mutexSet.count(edge) == 0 && m_mutexSet.size() != vec_edges.size())
            {
                ...
                for (const size_t triplet_index : vec_triplet_ordered)
                {
                    ...
                    if (m_mutexSet.count(Pair(triplet.i, triplet.j)) && m_mutexSet.count(Pair(triplet.i, triplet.k)) &&
                    ...
                    const bool bTriplet_estimation = Estimate_T_triplet(sfmData,
                                                                        map_globalR,
                    ...
                    if (bTriplet_estimation)
                    {
                        ...
                        {
                            const Mat3 RI = map_globalR.at(triplet.i), RJ = map_globalR.at(triplet.j), RK = map_globalR.at(triplet.k);
                            const Vec3 ti = vec_tis[0], tj = vec_tis[1], tk = vec_tis[2];
                            ...
                            initial_estimates[thread_id].emplace_back(std::make_pair(triplet.i, triplet.j), std::make_pair(Rij, tij));
                            initial_estimates[thread_id].emplace_back(std::make_pair(triplet.j, triplet.k), std::make_pair(Rjk, tjk));
                            initial_estimates[thread_id].emplace_back(std::make_pair(triplet.i, triplet.k), std::make_pair(Rik, tik));

//--- ATOMIC
#pragma omp critical
                            {
                                // Add inliers as valid pairwise matches
                                for (std::vector<size_t>::const_iterator iterInliers = vec_inliers.begin(); iterInliers != vec_inliers.end();
                                     ++iterInliers)
                                {
    ...
    ALICEVISION_LOG_DEBUG("-------------------------------\n"
                          ...
                          << m_vec_initialRijTijEstimates.size() / 3 << " computed from " << vec_triplets.size()
                          << " triplets.\n"
                             "-- resulting in "
                          << m_vec_initialRijTijEstimates.size()
...
bool GlobalSfMTranslationAveragingSolver::Estimate_T_triplet(const SfMData& sfmData,
                                                             const HashMap<IndexT, Mat3>& map_globalR,
                                                             const feature::FeaturesPerView& normalizedFeaturesPerView,
                                                             const matching::PairwiseMatches& pairwiseMatches,
                                                             ...
                                                             const std::string& outDirectory) const
{
    ...
    // Get rotations:
    const std::vector<Mat3> vec_global_R_Triplet = {map_globalR.at(poses_id.i), map_globalR.at(poses_id.j), map_globalR.at(poses_id.k)};
...
#ifdef DEBUG_TRIPLET
    {
        ALICEVISION_LOG_DEBUG("Triplet : status: " << bTest << " AC: " << dPrecision << " inliers % "
                                                   << double(vec_inliers.size()) / tracks.size() * 100.0 << " total putative " << tracks.size());
    }
#endif
