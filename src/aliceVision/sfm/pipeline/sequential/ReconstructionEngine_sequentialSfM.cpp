...
bool ReconstructionEngine_sequentialSfM::Process()
{
  ...
  {
    // could triangulate valid poses if it makes sense
...
#pragma omp critical
    ...
    if (relativePoseSuccess && relativePose_info.vec_inliers.size() > iMin_inliers_count)
    {
      ...
      for (const size_t inlier_idx: relativePose_info.vec_inliers)
      {
        ...
        vec_angles[i] = AngleBetweenRay(pose_I, camI, pose_J, camJ, featI, featJ);
...
bool ReconstructionEngine_sequentialSfM::checkAngles(const Vec3& pt3D, const std::set<IndexT>& viewsId, const SfMData& scene, double kMinAngle)
{
