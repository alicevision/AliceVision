...
double computeCameraScore(const SfMData& sfmData, const track::TracksPerView& tracksPerView, IndexT viewId)
{
    ...
    for (auto landmarkId : viewLandmarks)
    {
        ...
        sfmData::Observations::const_iterator itObs = landmark.getObservations().find(viewId);
...
void RigSequence::setupRelativePoses()
{
    ...
    std::vector<IndexT> subPosesToInitialized;
    {
        ...
        {
            if (subPoseInfoPair.second.nbPose > _params.minNbCamerasForCalibration && !subPoseInfoPair.second.isInitialized)
    ...
    for (auto& frameInfoPair : _rigInfoPerFrame)
    {
        ...
        if (_sfmData.getPoses().find(rigPoseId) == _sfmData.getPoses().end())
        {
            ...
            for (const auto& viewInfoPair : rigFrame)
            {
                ...
                if (itSubPoseInfoPair != _rigInfoPerSubPose.end() && itSubPoseInfoPair->second.isInitialized && viewInfo.score > bestScore)
                {
        ...
        for (auto& viewInfoPair : rigFrame)
        {
            ...
            if (!viewInfo.isPoseIndependant || itSubPoseInfoPair == _rigInfoPerSubPose.end() || !itSubPoseInfoPair->second.isInitialized)
                continue;
