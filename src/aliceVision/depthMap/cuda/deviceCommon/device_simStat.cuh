...
struct simStat
{
    ...
    inline __device__ simStat& operator=(const simStat& param)
    {
        xxsum = param.xxsum;
        yysum = param.yysum;
        xysum = param.xysum;
    ...
    inline __device__ float getVarianceX() { return (xxsum / count - (xsum * xsum) / (count * count)); }

    inline __device__ float getVarianceY() { return (yysum / count - (ysum * ysum) / (count * count)); }

    inline __device__ float getVarianceXY() { return (xysum / count - (xsum * ysum) / (count * count)); }

    /**
    * @brief Variance of X
    * formula: sum(w*x*x) / sum(w) - sum(w*x)^2 / sum(w)^2
    */
    inline __device__ float getVarianceXW()
    {
    ...
    */
    inline __device__ float getVarianceYW()
    {
     ...
     */
    inline __device__ float getVarianceXYW()
    {
     ...
     */
    inline __device__ void computeSimSub(simStat& ss)
    {
        // NCC
    ...
    inline __device__ void computeSim()
    {
        // NCC
    ...
    inline __device__ void computeWSim()
    {
        // NCC
    ...
    inline __device__ void update(const float2 g)
    {
        count += 1.0f;
        xsum += g.x;
        ysum += g.y;
    ...
    inline __device__ void update(const float2 g, float w)
    {
        wsum += w;
        count += 1.0f;
        xsum += w * g.x;
    ...
    inline __device__ void update(const float gx, const float gy)
    {
        count += 1.0f;
        xsum += gx;
        ysum += gy;
    ...
    inline __device__ void update(const float gx, const float gy, float w)
    {
        wsum += w;
        count += 1.0f;
        xsum += w * gx;
    ...
    inline __device__ void update(const uchar4 c1, const uchar4 c2)
    {
        float2 g;
    ...
    inline __device__ void update(const float3 c1, const float3 c2)
    {
        float2 g;
        g.x = c1.x;
        g.y = c2.x;
