...
void DepthSimMap::saveToImage(const std::string& filename, float simThr) const
{
    ...
    try
    {
        ...
        writeImage(_mp.fs, filename, bufferWidth, _h, colorBuffer, EImageQuality::LOSSLESS, OutputFileColorSpace(EImageColorSpace::NO_CONVERSION), metadata);
    }
    catch (...)
    {
...
void DepthSimMap::save(const std::string& customSuffix, bool useStep1) const
{
    ...
    using namespace imageIO;
    writeImage(_mp.fs, getFileNameFromIndex(_mp, _rc, mvsUtils::EFileType::depthMap, _scale, customSuffix),
               width, height, depthMap.getDataWritable(), EImageQuality::LOSSLESS, OutputFileColorSpace(EImageColorSpace::NO_CONVERSION), metadata);
    writeImage(_mp.fs, getFileNameFromIndex(_mp, _rc, mvsUtils::EFileType::simMap, _scale, customSuffix),
               width, height, simMap.getDataWritable(), imageIO::EImageQuality::OPTIMIZED, OutputFileColorSpace(EImageColorSpace::NO_CONVERSION), metadata);
}
...
void DepthSimMap::load(int fromScale)
{
    ...
    using namespace imageIO;
    readImage(_mp.fs, getFileNameFromIndex(_mp, _rc, mvsUtils::EFileType::depthMap, fromScale),
              width, height, depthMap.getDataWritable(), EImageColorSpace::NO_CONVERSION);
    readImage(_mp.fs, getFileNameFromIndex(_mp, _rc, mvsUtils::EFileType::simMap, fromScale),
              width, height, simMap.getDataWritable(), EImageColorSpace::NO_CONVERSION);
