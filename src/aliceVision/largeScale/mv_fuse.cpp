...
bool mv_fuse::filterGroupsRC(int rc, int pixSizeBall, int pixSizeBallWSP, int nNearestCams)
{
    ...
    {
        std::stringstream s;
        s << "filterGroupsRC: bad image dimension for camera: " << rc+1 << "\n";
...
bool mv_fuse::filterDepthMapsRC(int rc, int minNumOfModals, int minNumOfModalsWSP2SSP)
{
    ...
    imageIO::writeImage(mv_getFileName(mp->mip, rc + 1, EFileType::depthMap, 0), w, h, depthMap, imageIO::EImageQuality::LOSSLESS, metadata);
