 ...
 * @param[in] c the sycl::float3 sRGB
 * @return sycl::float3 linear RGB
 */
inline sycl::float3 srgb2rgb(const sycl::float3& c)
{
    return sycl::float3(c.x() <= 0.04045f ? c.x() / 12.92f : sycl::half_precision::powr((c.x() + 0.055f) / 1.055f, 2.4f),
                        c.y() <= 0.04045f ? c.y() / 12.92f : sycl::half_precision::powr((c.y() + 0.055f) / 1.055f, 2.4f),
                        c.z() <= 0.04045f ? c.z() / 12.92f : sycl::half_precision::powr((c.z() + 0.055f) / 1.055f, 2.4f));
}

/**
 * @brief Linear RGB (0..1) to XZY (0..1) using sRGB primaries
 * @param[in] c the sycl::float3 Linear RGB
 * @return sycl::float3 XYZ
 */
 ...
 * @param[in] c the sycl::float3 Linear RGB
 * @return sycl::float3 HSL
 */
inline sycl::float3 rgb2hsl(const sycl::float3& c)
{
    const float cmin = multi_fmin(c.x(), c.y(), c.z());
    const float cmax = sycl::fmax(c.x(), sycl::fmax(c.y(), c.z()));
 ...
 * @param[in] c the sycl::float3 XYZ
 * @return sycl::float3 CIELAB
 */
...
inline float CostYKfromLab(const int dx,
                           const int dy,
                           const sycl::float4 c1,
                           const sycl::float4 c2,
