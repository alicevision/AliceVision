...
struct distanceRatio
{
    ...
    inline bool isValid(const double distRatio) const
    {
        ...
        // - the distance ratio
        return (sbd != std::numeric_limits<DistT>::max() && bd < distRatio * sbd);
    }
};
                    ...
                    matching::IndMatches& vec_corresponding_index)
{
    ...
    for (Mat::Index i = 0; i < xLeft.cols(); ++i)
    {
        ...
        for (Mat::Index j = 0; j < xRight.cols(); ++j)
        {
            ...
            if (geomErr < errorTh)
            {
                const typename MetricT::ResultType descDist = metric(lDescriptors[i].getData(), rDescriptors[j].getData(), DescriptorT::static_size);
                    ...
                    matching::MatchesPerDescType& out_matchesPerDesc)
{
    ...
    {
        guidedMatching<ModelT, ErrorT>(
          mod, camL, *lRegions.at(descType), camR, *rRegions.at(descType), errorTh, distRatio, out_matchesPerDesc[descType]);
    }
}
