...
string_cmp(const void *p1, const void *p2)
{
    ...
    int compareTo(unsigned char * A, unsigned char * B )
    {
        for (int i = 0; i < 16; i++)
        {
            int tmp = string_cmp( A+i*8, B+i*8 );
            if (tmp != 0) return tmp;
        }
    ...
    for (int i=0; i < nbQuery; ++i)
        {
            ...
            if(pos >= 20 && pos <= (*memMapping).rows()-20) {
                for(int x = pos-10; x < pos+10; x++) {                    

                    //Scalar * test = (*memMapping).data();

                    //test+=(desc[x].idx*(*memMapping).cols());
                    //double num = metric(queryPtr, test, (*memMapping).cols());
                    double num = metric(queryPtr, test+(desc[x].idx*(*memMapping).cols()), (*memMapping).cols());
            ...
            (*pvec_distances)[i*NN] = best;
            (*pvec_indices)[i*NN] = IndMatch(i, bestIndex);
            (*pvec_distances)[i*NN+1] = nextBest;
