...
class ArrayMatcherBruteForce  : public ArrayMatcher<Scalar, Metric>
{
   ...
   )
  {
    ...
    int cnter = 0;
    int last = (*memMapping).rows() -1;
    int remaining = (*memMapping).rows();
    int lookup[(*memMapping).rows()];
    int lookup_index[(*memMapping).rows()];

    float fratio = 0.64f;
    //create lookup table for origional indexes as well
    //removed indexes does not exist in lookup, thus overlapping will not matter
	...
	const Scalar * rowPtr_start = (*memMapping).data();

	rowPtr += lookup[0] * (*memMapping).cols();
	...
	for (int i = 0; i < remaining; ++i) {//477 times
	  ...
	  rowPtr = rowPtr_start + lookup[i+1] * (*memMapping).cols();
	...
	(*pvec_indices)[queryIndex*NN+0] = IndMatch(queryIndex, lookup_index[bestIndex]);
	
	(*pvec_distances)[queryIndex*NN+1] = vec_distance[nextBestIndex];
	(*pvec_indices)[queryIndex*NN+1] = IndMatch(queryIndex, lookup_index[nextBestIndex]);

	//if (vec_distance[bestIndex] < vec_distance[nextBestIndex] * 0.64f)
//	std::cout << "distance: " << vec_distance[bestIndex] << " distance 2: " << (vec_distance[nextBestIndex]) << " fratio iter2: " << fratio * vec_distance[nextBestIndex] << std::endl;


	    if (vec_distance[bestIndex]  < vec_distance[nextBestIndex] * fratio )
	{
	    // std::cout << "distance larger more then 0.7 cnt: " << cnter++ << std::endl;
	    lookup[bestIndex] = lookup[last];
	    lookup_index[bestIndex] = last;
	    remaining--;
	    last--;  
	}   
