...
std::string describerTypeColor(feature::EImageDescriberType descType)
{
    switch (descType)
    {
        case feature::EImageDescriberType::SIFT:
            return "yellow";
        case feature::EImageDescriberType::SIFT_FLOAT:
            return "yellow";
        case feature::EImageDescriberType::SIFT_UPRIGHT:
            return "yellow";
        ...
        case feature::EImageDescriberType::CCTAG3:
            return "blue";
        case feature::EImageDescriberType::CCTAG4:
            return "blue";
#endif
#if ALICEVISION_IS_DEFINED(ALICEVISION_HAVE_APRILTAG)
        case feature::EImageDescriberType::APRILTAG16H5:
            return "blue";
#endif
        ...
        case feature::EImageDescriberType::UNKNOWN:
            return "red";
        case feature::EImageDescriberType::UNINITIALIZED:
            return "fuchsia";
    }
    return "magenta";
}
...
float getStrokeEstimate(const std::pair<size_t, size_t>& imgSize) { return std::max(std::max(imgSize.first, imgSize.second) / float(2200), 2.0f); }
                       ...
                       bool richKeypoint /*= true*/)
{
    ...
    for (const feature::PointFeature& kpt : keypoints)
    {
        ...
        if (richKeypoint)
        {
            ...
            svgStream.drawLine(kpt.x(), kpt.y(), point(0), point(1), svg::svgStyle().stroke("yellow", strokeWidth));
        }
    }
                              ...
                              bool left)
{
    ...
    else
    {
        ...
        if (!((point(0) > 0 && point(0) < imageSize.first) && (point(1) > 0 && point(1) < imageSize.second)))
        {
            ALICEVISION_LOG_DEBUG("epipole outside the image:\n" << point);
            // point outside the image, clamp it to the borders
            if (point(0) < 0)
                point(0) = 0;
            if (point(0) > imageSize.first)
                point(0) = imageSize.first;
            if (point(1) < 0)
                point(1) = 0;
            if (point(1) > imageSize.second)
                point(0) = imageSize.second;
                          ...
                          bool showNotMatched)
{
    ...
    for (const matching::IndMatch& m : matches)
    {
        ...
        {
            ALICEVISION_LOG_WARNING("[svg]\tWarning! cctagIdLeft " << cctagIdLeft << " "
                                                                   << "cctagIdRight " << cctagIdRight);
            continue;
        }
