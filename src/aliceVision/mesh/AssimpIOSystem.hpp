...
{
public:
    VfsIOSystem(vfs::filesystem& fs) : _fs{fs} { }
    ...
    Assimp::IOStream* Open(const char* path, const char* mode) override
    {
        ...
        if (mode[0] == 'r')
        {
            auto stream = _fs.open_read_binary(path);
            if (!stream.is_open())
                return nullptr;
        ...
        if (mode[0] == 'w')
        {
            auto stream = _fs.open_write_binary(path);
            if (!stream.is_open())
                return nullptr;
    ...
    {
        return _fs.equivalent(one, second);
    }

    bool CreateDirectory(const std::string &path) override
    {
        vfs::error_code ec;
        _fs.create_directory(path, ec);
        return !ec.failed();
    }
    ...
    {
        vfs::error_code ec;
        bool existed = _fs.remove(file, ec);
