...
{
    StaticVector<int>*& ptNeighPtsOrderedByPath = (*m_me->ptsNeighPtsOrdered)[ptId];
...
int MeshClean::path::deployAll()
{
    ...
    while(path.size() > 0)
    {
        ...
        else
        {
            ...
            (*m_me->ptsBoundary)[m_ptId] = (!isClodePath(pathNew));
            updatePtNeighPtsOrderedByPath(m_ptId, pathNew);
        }
    }
