...
void RobertsonCalibrate::process(const std::vector<std::vector<image::Image<image::RGBfColor>>>& ldrImageGroups,
                                 ...
                                 int nbPoints, bool fisheye, const rgbCurve& weight, rgbCurve& response)
{
        ...
        if(fisheye)
        {
            ...
            for(unsigned int j = 0; j < nbImages; ++j)
            {
                ...
                for(int y = yMin; y <= yMax - step; y += step)
                {
                    ...
                    {
                        const std::size_t dist2 = std::pow(center(0) - x, 2) + std::pow(center(1) - y, 2);
                        if(dist2 > maxDist2)
                            continue;
                        ...
                        for(std::size_t channel = 0; channel < channels; ++channel)
                        {
                            ...
                            std::size_t index =
                                std::round(clamp(pixelValue(channel), 0.f, 1.f) * (channelQuantization - 1));
            ...
            if(fisheye)
            {
                ...
                for(unsigned int j = 0; j < nbImages; ++j)
                {
                    ...
                    for(int y = yMin; y <= yMax - step; y += step)
                    {
                        for(int x = xMin; x <= xMax - step; x += step)
                        {
                            const std::size_t dist2 = pow(center(0) - x, 2) + pow(center(1) - y, 2);
                            if(dist2 > maxDist2)
                                continue;
...
#pragma omp parallel for
        ...
        const double diff = rgbCurve::sumAll(responseDiff) / channels;
        ...
        if(diff < _threshold)
        {
            ALICEVISION_LOG_ERROR("[RobertsonCalibrate] difference < threshold"
                                  " (difference: " << diff << ", threshold: " << _threshold << ")");
            break;
        }
