...
class ImageDescriber_DSPSIFT_vlfeat : public ImageDescriber
{
  public:
    ...
    ~ImageDescriber_DSPSIFT_vlfeat() override { VLFeatInstance::destroy(); }
     ...
     */
    EImageDescriberType getDescriberType() const override { return EImageDescriberType::DSPSIFT; }
     ...
     */
    void setConfigurationPreset(ConfigurationPreset preset) override { _params.setPreset(preset); }
     ...
     */
    void allocate(std::unique_ptr<Regions>& regions) const override { regions.reset(new SIFT_Regions); }
