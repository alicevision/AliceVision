                 ...
                 const image::Image<unsigned char>* mask)
{
    ...
    while (true)
    {
             ...
             params._contrastFiltering == EFeatureConstrastFiltering::GridSortOctaves))
        {
            ...
            if (nkeys > maxOctaveKeypoints)
            {
                ...
                if (filteredKeypointsIndex.size() < params._maxTotalKeypoints && !rejected_indexes.empty())
                {
                    ...
                    filteredKeypointsIndex.insert(
                      filteredKeypointsIndex.end(), rejected_indexes.begin(), rejected_indexes.begin() + remainingElements);
                }
        ...
        else if (params._maxTotalKeypoints && params._contrastFiltering == EFeatureConstrastFiltering::NonExtremaFiltering)
        {
            ...
            std::partial_sort(filteredKeypointsIndex.begin(),
                              ...
                              filteredKeypointsIndex.end(),
                              [&](int a, int b) { return radiusMaxima[a] * keys[a].sigma > radiusMaxima[b] * keys[b].sigma; });
...
#pragma omp critical
    ...
    if (params._maxTotalKeypoints && params._contrastFiltering == EFeatureConstrastFiltering::NonExtremaFiltering)
    {
        ...
        if (features.size() > params._maxTotalKeypoints)
        {
            ...
            std::partial_sort(indexSort.begin(),
                              indexSort.begin() + std::min(params._maxTotalKeypoints, features.size()),
                              indexSort.end(),
                              [&](int a, int b) { return radiusMaxima[a] * features[a].scale() > radiusMaxima[b] * features[b].scale(); });
            ...
            ALICEVISION_LOG_TRACE("SIFT Features: before: " << features.size() << ", after grid filtering: " << filteredFeatures.size());
    ...
    ALICEVISION_LOG_TRACE("SIFT Features: " << regionsCasted->Features().size() << " (max: " << params._maxTotalKeypoints << ").");
    assert(regionsCasted->Features().size() == regionsCasted->Descriptors().size());

    return true;
}

template bool extractSIFT<float>(const image::Image<float>& image,
                                 std::unique_ptr<Regions>& regions,
                                 const SiftParams& params,
                                 bool orientation,
                                 const image::Image<unsigned char>* mask);

template bool extractSIFT<unsigned char>(const image::Image<float>& image,
                                         std::unique_ptr<Regions>& regions,
                                         const SiftParams& params,
                                         bool orientation,
                                         const image::Image<unsigned char>* mask);
