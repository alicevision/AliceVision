...
static inline std::unique_ptr<feature::Regions> Init_region_type_from_file
(
  const std::string & sImageDescriber_file
)
{
  using namespace aliceVision::feature;
  std::unique_ptr<Regions> regions_type;
  if (stlplus::is_file(sImageDescriber_file))
  {
    // Dynamically load the regions type from the file
    std::ifstream stream(sImageDescriber_file.c_str());
    if (stream.is_open())
    {
  ...
  {
    regions_type.reset(new feature::SIFT_Regions());
  }
  return regions_type;
}
