...
{
    if (imageDescriberPreset == EImageDescriberPreset::LOW)
        return "low";
    if (imageDescriberPreset == EImageDescriberPreset::MEDIUM)
        return "medium";
    if (imageDescriberPreset == EImageDescriberPreset::NORMAL)
        return "normal";
    if (imageDescriberPreset == EImageDescriberPreset::HIGH)
        return "high";
    if (imageDescriberPreset == EImageDescriberPreset::ULTRA)
        return "ultra";

    throw std::invalid_argument("Unrecognized EImageDescriberPreset: " + std::to_string(int(imageDescriberPreset)));
}

std::ostream& operator<<(std::ostream& os, EImageDescriberPreset p) { return os << EImageDescriberPreset_enumToString(p); }
...
std::ostream& operator<<(std::ostream& os, EFeatureQuality p) { return os << EFeatureQuality_enumToString(p); }
...
std::ostream& operator<<(std::ostream& os, EFeatureConstrastFiltering p) { return os << EFeatureConstrastFiltering_enumToString(p); }
...
std::unique_ptr<ImageDescriber> createImageDescriber(EImageDescriberType imageDescriberType)
{
    ...
    switch (imageDescriberType)
    {
        case EImageDescriberType::SIFT:
            describerPtr.reset(new ImageDescriber_SIFT(SiftParams(), true));
            break;
        case EImageDescriberType::SIFT_FLOAT:
            describerPtr.reset(new ImageDescriber_SIFT_vlfeatFloat(SiftParams()));
            break;
        case EImageDescriberType::SIFT_UPRIGHT:
            describerPtr.reset(new ImageDescriber_SIFT(SiftParams(), false));
            break;

        case EImageDescriberType::DSPSIFT:
            describerPtr.reset(new ImageDescriber_DSPSIFT_vlfeat(DspSiftParams(), true));
            break;

        case EImageDescriberType::AKAZE:
            describerPtr.reset(new ImageDescriber_AKAZE(AKAZEParams(AKAZEOptions(), feature::AKAZE_MSURF)));
            break;
        case EImageDescriberType::AKAZE_MLDB:
            describerPtr.reset(new ImageDescriber_AKAZE(AKAZEParams(AKAZEOptions(), feature::AKAZE_MLDB)));
            break;
        ...
        case EImageDescriberType::CCTAG3:
            describerPtr.reset(new ImageDescriber_CCTAG(3));
            break;
        case EImageDescriberType::CCTAG4:
            describerPtr.reset(new ImageDescriber_CCTAG(4));
            break;
#endif  // ALICEVISION_HAVE_CCTAG

#if ALICEVISION_IS_DEFINED(ALICEVISION_HAVE_APRILTAG)
        case EImageDescriberType::APRILTAG16H5:
            describerPtr.reset(new ImageDescriber_APRILTAG());
            break;
#endif  // ALICEVISION_HAVE_APRILTAG
