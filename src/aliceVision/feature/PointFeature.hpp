...
class PointFeature
{
  ...
  public:
    ...
    {
        return (_scale == b.scale()) && (_orientation == b.orientation()) && (x() == b.x()) && (y() == b.y());
    }

    bool operator!=(const PointFeature& b) const { return !((*this) == b); }
...
inline void loadFeatsFromFile(const std::string& sfileNameFeats, FeaturesT& vec_feat)
{
    ...
    std::copy(std::istream_iterator<typename FeaturesT::value_type>(fileIn),
...
/// Write feats to file
template<typename FeaturesT>
inline void saveFeatsToFile(const std::string& sfileNameFeats, FeaturesT& vec_feat)
{
...
void PointsToMat(const FeaturesT& vec_feats, MatT& m)
{
    m.resize(2, vec_feats.size());
    typedef typename FeaturesT::value_type ValueT;  // Container type

    size_t i = 0;
    for (typename FeaturesT::const_iterator iter = vec_feats.begin(); iter != vec_feats.end(); ++iter, ++i)
    {
