...
DescriptorExtractor_LIOP::DescriptorExtractor_LIOP() { GeneratePatternMap(m_LiopPatternMap, m_LiopPosWeight, _liopNum); }
...
inline bool fGrayComp(const Pixel& p1, const Pixel& p2) { return p1.f_gray < p2.f_gray; }
...
bool BilinearInterpolation_BorderCheck(float& val, float x, float y, const image::Image<float>& image, const image::Image<unsigned char>& flagImage)
{
    ...
    val = (x2 - x) * (y2 - y) * data[y1 * step + x1] + (x - x1) * (y2 - y) * data[y1 * step + x2] + (x2 - x) * (y - y1) * data[y2 * step + x1] +
...
void DescriptorExtractor_LIOP::CreateLIOP_GOrder(const image::Image<float>& outPatch,
                                                 ...
                                                 float desc[144]) const
{
    ...
    Pixel pixel[_maxPixelNum];
    int pixelCount = 0;
    int idx[_maxSampleNum];
    float dst[_maxSampleNum];
    float src[_maxSampleNum];
    ...
    for (int y = -inRadius; y <= inRadius; ++y)
    {
        for (int x = -inRadius; x <= inRadius; ++x)
        {
            ...
            int key = 0;
            SortGray(dst, idx, src, _liopNum);
            for (int k = 0; k < _liopNum; ++k)
            {
                key += (idx[k] + 1) * m_LiopPosWeight[_liopNum - k - 1];
            }
    ...
    const int l_patternWidth = _liopNum == 3 ? 6 : 24;
    const int dim = l_patternWidth * _regionNum;

    if (pixelCount >= _regionNum)
    {
        int curId = 0;
        int lastId = 0;
        for (int i = 0; i < _regionNum; ++i)
        {
            const int fenceId = pixelCount * (i + 1) / _regionNum - 1;
...
void DescriptorExtractor_LIOP::extract(const image::Image<float>& I, const PointFeature& feat, float desc[144])
{
...
void DescriptorExtractor_LIOP::GeneratePatternMap(std::map<int, unsigned char>& pattern_map, std::vector<int>& pos_weight, unsigned char n)
{
    pattern_map.clear();
    pos_weight.resize(n);
