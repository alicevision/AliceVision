...
const float gauss25[7][7] = {{0.02546481f, 0.02350698f, 0.01849125f, 0.01239505f, 0.00708017f, 0.00344629f, 0.00142946f},
...
void AKAZE::computeScaleSpace()
{
    ...
    for (int p = 0; p < _options.nbOctaves; ++p)
    {
        ...
        for (int q = 0; q < _options.nbSlicePerOctave; ++q)
        {
            ...
            computeAKAZESlice(input, p, q, _options.nbSlicePerOctave, _options.sigma0, contrastFactor, evo.cur, evo.Lx, evo.Ly, evo.Lhess);
...
void detectDuplicates(std::vector<std::pair<AKAZEKeypoint, bool>>& previous, std::vector<std::pair<AKAZEKeypoint, bool>>& current)
{
        ...
        for (int q = 0; q < _options.nbSlicePerOctave; ++q)
        {
            ...
            for (int jx = borderLimit; jx < LDetHess.height() - borderLimit; ++jx)
            {
                for (int ix = borderLimit; ix < LDetHess.width() - borderLimit; ++ix)
                {
                    ...
                    if (value > _options.threshold && value > LDetHess(jx - 1, ix) && value > LDetHess(jx - 1, ix + 1) &&
                        value > LDetHess(jx - 1, ix - 1) && value > LDetHess(jx, ix - 1) && value > LDetHess(jx, ix + 1) &&
                        value > LDetHess(jx + 1, ix - 1) && value > LDetHess(jx + 1, ix) && value > LDetHess(jx + 1, ix + 1))
                    {
                        AKAZEKeypoint point;
...
void AKAZE::computeMainOrientation(AKAZEKeypoint& keypoint, const image::Image<float>& Lx, const image::Image<float>& Ly) const
{
    ...
    {
        ang2 = (ang1 + M_PI / 3.0f > 2.0f * M_PI ? ang1 - 5.0f * M_PI / 3.0f : ang1 + M_PI / 3.0f);
        sumX = sumY = 0.f;
        ...
        for (std::size_t k = 0; k < idx; ++k)
        {
            ...
            else if (ang2 < ang1 && ((ang > 0 && ang < ang2) || (ang > ang1 && ang < 2.0f * M_PI)))
            {
                sumX += resX[k];
                sumY += resY[k];
            }
