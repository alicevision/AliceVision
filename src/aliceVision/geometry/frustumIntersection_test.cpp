...
BOOST_AUTO_TEST_CASE(intersection)
{
    ...
    const NViewDataSet d = NRealisticCamerasRing(iNviews, iNbPoints, NViewDatasetConfigurator(focal, focal, principal_Point, principal_Point, 5, 0));
    ...
    // Test with infinite Frustum for each camera
    {
        ...
        {
            vec_frustum.push_back(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i], d._C[i]));
    ...
    // Test with truncated frustum
    {
        ...
        for (int i = 0; i < iNviews; ++i)
        {
            ...
            vec_frustum.push_back(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i], d._C[i], minDepth, maxDepth));
    ...
    // Test with partially truncated frustum
    {
        ...
        for (int i = 0; i < iNviews; ++i)
        {
            vec_frustum.push_back(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i], d._C[i], 0.1, -1.0));
...
BOOST_AUTO_TEST_CASE(empty_intersection)
{
    ...
    // Test with infinite Frustum for each camera
    {
        ...
        for (int i = 0; i < iNviews; ++i)
        {
            ...
            vec_frustum.push_back(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i] * flipMatrix, d._C[i]));
            BOOST_CHECK(vec_frustum[i].isInfinite());
        }
    ...
    // Test but with truncated frustum
    {
        ...
        for (int i = 0; i < iNviews; ++i)
        {
            ...
            const Mat3 flipMatrix = RotationAroundY(degreeToRadian(180.0));
            vec_frustum.push_back(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i] * flipMatrix, d._C[i], minDepth, maxDepth));
            BOOST_CHECK(vec_frustum[i].isTruncated());
        }
...
void createPanoramaScene(double coeff, NViewDataSet& d, const int iNviews, const int iNbPoints, const int principal_Point)
{
    ...
    for (size_t i = 0; i < iNviews; ++i)
    {
        ...
        d._K[i] << config._fx, 0, config._cx, 0, config._fy, config._cy, 0, 0, 1;
...
BOOST_AUTO_TEST_CASE(panorama_intersection)
{
    ...
    // Test with partially truncated frustum
    {
        ...
        for (int i = 0; i < iNviews; ++i)
        {
            vec_frustum.push_back(Frustum(principal_Point * 2, principal_Point * 2, d._K[i], d._R[i], d._t[i], 0.1, -1.0));
            BOOST_CHECK(vec_frustum[i].isPartiallyTruncated());
        }
