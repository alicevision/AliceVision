...
class SfMExpanding(desc.AVCommandLineNode):
    ...
    inputs = [
        ...
        desc.FloatParam(
            name="tscTrackLengthRegWeight",
            label="Track Length Based Regularization Weight",
            description="Controls the strength of a regularization applied to the temporal constraint, encouraging the track length to remain constant.",
            value=0.0,
            range=(0.0, 100.0, 0.1),
            advanced=True,
            enabled=lambda node: node.useTemporalConstraint.value,
