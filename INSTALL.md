...
The Dockerfile enables to build a Ubuntu based image based on `nvidia/cuda` image (https://hub.docker.com/r/nvidia/cuda/)

A parameter `CUDA_TAG` can be passed when building the image to select the Ubuntu and the CUDA version.
For example, in order to create a Ubuntu 16.04 with Cuda 8.0 for development, use
...
```
docker run -it --runtime=nvidia alicevision:$CUDA_TAG
```

Note that a CentOS image can be also generated by replacing the `apt-get install` part with an equivalent `yum install ...` line and the relevant required packages.
Then, the `CUDA_TAG` parameter can be set to select the Cuda and CentOS version, e.g.

```
CUDA_TAG=8.0-devel-centos6
```

to create a CentOS 6.0 image with Cuda 8.0.
