...
def test_sfmdata_get_views():
    ...
    assert (
        len(views) == 0
    ), "The SfMData object is empty, there should not be any View in it"

    # Create a View object and add it to the list
    view = av.View()
    views[VIEW_ID] = view
    assert (
        len(data.getViews()) == len(views) == 1
    ), "The list of Views should have been updated"
...
def test_sfmdata_get_poses():
    ...
    assert (
        len(poses) == 0
    ), "The SfMData object is empty, there should not be any Pose in it"

    # Create a Pose object and add it to the list
    camera_pose = av.CameraPose()
    poses[POSE_ID] = camera_pose
    assert (
        len(data.getPoses()) == len(poses) == 1
    ), "The list of Poses should have been updated"
...
def test_sfmdata_get_rigs():
    ...
    assert (
        len(rigs) == 0
    ), "The SfMData object is empty, there should not be any Rig in it"

    # Create a Rig object and add it to the list
    rig = av.Rig(5)
    rigs[RIG_ID] = rig
    assert (
        len(data.getRigs()) == len(rigs) == 1
    ), "The list of Rigs should have been updated"
...
def test_sfmdata_get_intrinsics():
    ...
    assert (
        len(data.getIntrinsics()) == len(intrinsics) == 1
...
def test_sfmdata_get_landmarks():
    ...
    assert (
        len(data.getLandmarks()) == len(landmarks) == 1
...
def test_sfmdata_get_constraints2d():
    ...
    assert (
        len(data.getConstraints2D()) == len(constraints) == 1
...
def test_sfmdata_get_rotationpriors():
    ...
    priors.append(prior)
    assert (
        len(data.getRotationPriors()) == len(priors) == 1
...
def test_sfmdata_get_views_keys():
    ...
    assert len(data.getViews()) == len(
        view_ids
    ), "The SfMData object should not be empty anymore"

    # Check the keys
    keys = data.getViewsKeys()
    assert len(keys) == len(
        view_ids
    ), "There should be as many View objects as view IDs"
    key_checks = [view_id in keys for view_id in view_ids]
    assert all(key_checks), "All the view IDs should be in the list of View keys"
...
def test_sfmdata_get_view():
    ...
    for i in range(5):
        ...
        assert view.getLightId() == LIGHT_ID + i
...
def test_sfmdata_combine():
    ...
    assert len(data2.getViews()) == len(
        view_ids2
    ), "The SfMData object should contain several View objects"

    # Combine the two SfMData objects
    data1.combine(data2)
    assert len(data1.getViews()) == len(view_ids1) + len(
        view_ids2
    ...
    data3.combine(data1)
    data3.combine(data2)
    assert len(data3.getViews()) == len(view_ids1) + len(
        view_ids2
...
def test_sfmdata_get_set_folders():
    ...
    assert os.path.isabs(
        data.getFeaturesFolders()[0]
    ), "The absolute path has been set: the features folder's path should be absolute as well"
    assert (
        os.path.relpath(data.getFeaturesFolders()[0], projectFolder) == relative_folder
    ...
    assert os.path.isabs(
        data.getMatchesFolders()[0]
    ), "The absolute path has been set: the matches folder's path should be absolute as well"
    assert (
        os.path.relpath(data.getMatchesFolders()[0], projectFolder) == relative_folder
    ...
    assert (
        len(data.getRelativeFeaturesFolders()) == 1
        and data.getRelativeFeaturesFolders()[0] == relative_folder
    )
    assert (
        len(data.getRelativeMatchesFolders()) == 1
        and data.getRelativeMatchesFolders()[0] == relative_folder
    )
    ...
    for i in range(len(data.getFeaturesFolders())):
        assert (
            os.path.relpath(data.getFeaturesFolders()[i], projectFolder)
            == new_folders[i]
        )
        assert (
            os.path.relpath(data.getMatchesFolders()[i], projectFolder)
            == new_folders[i]
        )
