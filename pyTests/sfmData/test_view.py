...
def test_view_default_constructor():
    ...
    assert (
        view.getAncestors() == ()
    ), "Default View object should have an empty list of ancestors"
    ...
    assert (
        ...
        == view.getSubPoseId()
        == view.getViewId()
    ), "All IDs should be uninitialized"

    image_info = view.getImage()
    assert (
        image_info
    ), "An ImageInfo object should be available for a default View object"
...
def test_view_constructor():
    ...
    assert image_info.getImgSize() == (
        IMAGE_WIDTH,
        IMAGE_HEIGHT,
    ), "Image size does not correspond to the one set in constructor"
    assert (
        image_info.getImagePath() == IMAGE_PATH
    ...
    assert view.getLightId() == LIGHT_ID, "Unexpected light ID"
...
def test_view_get_set_values():
    ...
    assert (
        not view.isPartOfRig()
    ), "Rig ID is not defined, the View object cannot be part of a rig"
    ...
    view.setLightId(987654)
    assert view.getLightId() == 987654, "Light ID has not been correctly set"
...
def test_view_get_image():
    ...
    assert (
        image_info1 == image_info2
    ), "The ImageInfo object should be the same no matter \
        the way used to retrieve it"

    assert (
        image_info1.getWidth() == IMAGE_WIDTH
        and image_info1.getHeight() == IMAGE_HEIGHT
    ), "The ImageInfo object size should be the one set when creating the View object"
    assert image_info2.getImgSize() == (
        IMAGE_WIDTH,
        IMAGE_HEIGHT,
    ), "The ImageInfo object size should be the one set when creating the View object"
